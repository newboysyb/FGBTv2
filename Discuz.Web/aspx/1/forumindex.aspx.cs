	...
	public class forumindex : PageBase
    {
        ...
        protected override void ShowPage()
		{
            ...
            if (DNTRequest.GetString("s") == "login")
            {
                try
                {
                    HttpContext.Current.Response.Redirect(BaseConfigs.GetForumPath + "forumindex.aspx");
                }
                catch (System.Exception ex)
                {
                    SetUrl("forumindex.aspx");
                    SetMetaRefresh();
                    SetShowBackLink(false);
                    PTLog.InsertSystemLog(PTLog.LogType.ForumIndex, PTLog.LogStatus.Detail, "IndexLogin", ex.ToString());
                }
                return;
            }
			...
			forumlist = Forums.GetForumIndexCollection(config.Hideprivate, usergroupid, config.Moddisplay, out totaltopic, out totalpost, out todayposts, ipaddress);
            ...
            //【BT修改】修复必须点击详细在线列表才能更新最高用户数的BUG
            if (totalonline > TypeConverter.StrToInt(highestonlineusercount))
            {
                if (Statistics.UpdateStatistics("highestonlineusercount", totalonline.ToString()) > 0)
                {
                    Statistics.UpdateStatistics("highestonlineusertime", DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss"));
                    Statistics.ReSetStatisticsCache();
                    highestonlineusercount = totalonline.ToString();
                    highestonlineusertime = DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss");
                }
            }
