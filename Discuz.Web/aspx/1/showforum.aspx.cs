...
namespace Discuz.Web
{
    ...
    public class showforum : PageBase
    {
        ...
        public List<AbtSeedInfo> abtseedlist = new List<AbtSeedInfo>();
        ...
        protected override void ShowPage()
        {
            ...
            //除去admin账户外，全部跳转
            if (userid > 1 && PrivateBT.Forum2Type(forumid) > 0 && PrivateBT.Forum2Type(forumid) < 20)
            {
                System.Web.HttpContext.Current.Response.Redirect("showseeds.aspx?type=" + PrivateBT.Type2Str(PrivateBT.Forum2Type(forumid)));
                return;
            }
            ...
            if (forum.Subforumcount > 0)
            {
                subforumlist =
                    Forums.GetSubForumCollection(forumid, forum.Colcount, config.Hideprivate, usergroupid,
                                                 config.Moddisplay, ipaddress);
                
                //种子列表区不显示版块描述
                foreach (IndexPageForumInfo f in subforumlist)
                {
            ...
            //获取主题总数
            topiccount = Topics.GetTopicCount(new TopicListFilterInfo(forumid, -1, -1, topictypeid, Forums.GetTopicSpecialID(filter), interval));
            ...
            if (forum.Layer > 0)
            {
                ...
                //是否是Abt种子发布页面，是的话获取Abt种子列表
                if ((forum.Allowpostspecial & 64) == 64)
                {
                    abtseedlist = PTAbt.AbtGetSeedInfoList(topiclist);
                }
            }
            #endregion
        ...
        private void SetPageNumber()
        {
            if (DNTRequest.GetString("search") == "")
            {
                if (topictypeid == -1)
                {
                    if (config.Aspxrewrite == 1)
                    {
                        if (Utils.StrIsNullOrEmpty(filter))
                        {
                            ...
                            if (pageid < pagecount)
                            {
                                ...
                                nextpageurl = forumpath + Urls.ShowForumAspxRewrite(forumid, pageid + 1, forum.Rewritename);
                            }
                        }
                        else
                        {
                            ...
                            if (pageid < pagecount)
                            {
                                ...
                                nextpageurl = string.Format("{0}showforum.aspx?forumid={1}&filter={2}&page={3}", forumpath, forumid, filter, pageid + 1);
                            }
                        }
                    }
                    else
                    {
                        ...
                        if (pageid < pagecount)
                        {
                            ...
                            nextpageurl = string.Format("{0}showforum.aspx?forumid={1}{2}&page={3}", forumpath, forumid, (Utils.StrIsNullOrEmpty(filter) ? "" : "&filter=" + filter), pageid + 1);
                        }
                    }
                }
                else //当有主题类型条件时
                {
                    ...
                    if (pageid < pagecount)
                    {
                        ...
                        nextpageurl = string.Format("{0}showforum.aspx?forumid={1}&typeid={2}{3}&page={4}", forumpath, forumid, topictypeid, (Utils.StrIsNullOrEmpty(filter) ? "" : "&filter=" + filter), pageid + 1);
                    }
                }
            }
            else
            {
                ...
                if (pageid < pagecount)
                {
                    ...
                    nextpageurl = string.Format("{0}showforum.aspx?search=1&cond={1}&order={2}&direct={3}&forumid={4}&interval={5}&typeid={6}{7}&page={8}",
                              forumpath, Utils.HtmlEncode(DNTRequest.GetString("cond").Trim()), order, direct,
                              forumid, interval, topictypeid, (Utils.StrIsNullOrEmpty(filter) ? "" : "&filter=" + filter), pageid + 1);
                }
            }
