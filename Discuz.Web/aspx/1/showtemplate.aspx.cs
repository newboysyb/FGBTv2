    ...
    public class showtemplate : PageBase
    {
        protected override void ShowPage()
        {
            ...
            if (templateid > 0)
            {
                ...
                if (templateid == 5)
                {
                    if (hostname != "m.buaabt.cn" && hostname != "mbuaa.buaabt.cn" && hostname != "m1.buaabt.cn" && hostname != "m2.buaabt.cn")
                    {
                        AddErrLine("手机版模板只能在移动设备上使用！");
                        return;
                    }
                    if (!agent.Contains("Windows NT") && !agent.Contains("Macintosh"))
                    {
                        if (agent.Contains("Android") || agent.Contains("iPhone") || agent.Contains("iPod") || agent.Contains("iPad") || agent.Contains("Windows Phone") || agent.Contains("MQQBrowser"))
                        {
                            //strTemplateid = "5";
                        }
                        else
                ...
                //临时修改，防止Referer: "></meta><ScRipt>alert(1);</ScRipt>
