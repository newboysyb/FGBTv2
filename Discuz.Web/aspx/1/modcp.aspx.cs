    ...
    public class modcp : PageBase
    {
        ...
        private void Login()
        {
            ...
            if (needshowlogin)
            {
                ...
                //新密码体系验证用户密码，暂时为原始密码
                uid = PTUsers.CheckPasswordSHA512(username, password, true, false, "", true);
        ...
        public void UpdateForum()
        {
            ...
            if (!DoValidateVerifyCode()) return;

            ForumInfo foruminfo = Forums.GetForumInfo(forumid);
            if (useradminid != 3 || foruminfo.Alloweditrules != 0)
            {
                foruminfo.RulesUBB = Utils.HtmlEncode(DNTRequest.GetString("rulesmessage"));
                foruminfo.RulesIsHTML = DNTRequest.GetInt("forumrulesishtml_radio", 0);
            }
            foruminfo.DescriptionUBB = Utils.HtmlEncode(DNTRequest.GetString("descriptionmessage"));
            foruminfo.DescriptionIsHTML = DNTRequest.GetInt("forumdescriptionishtml_radio", 0);

            //UBB->HTML
            if(PrivateBT.Forum2Type(foruminfo.Fid) <= 0) foruminfo.DescriptionIsHTML = 0;
            if (foruminfo.RulesIsHTML == 0) foruminfo.Rules = UBB.ParseSimpleUBB(foruminfo.RulesUBB);
            if (foruminfo.DescriptionIsHTML == 0) foruminfo.Description = UBB.ParseSimpleUBB(foruminfo.DescriptionUBB);

            //如果是HTML格式版头
            if (useradminid == 1 && userid == 1 && username == "admin")
            {
                if (foruminfo.RulesIsHTML > 0) foruminfo.Rules = UBB.ParseSimpleUBB(Utils.HtmlDecode(foruminfo.RulesUBB));
                if (foruminfo.DescriptionIsHTML > 0) foruminfo.Description = UBB.ParseSimpleUBB(Utils.HtmlDecode(foruminfo.DescriptionUBB));
            }
            else
            {
                if (foruminfo.RulesIsHTML > 0) foruminfo.RulesIsHTML = 2;
                if (foruminfo.DescriptionIsHTML > 0) foruminfo.DescriptionIsHTML = 2;
            }
        ...
        //////////////////////////////////////////////////////////////////////////
        //*****modcp.aspx页面的功能是ajax实现的？？？相关审核通过的代码在Discuz.Web.UI.SessionAjaxPage.cs页面*********
        //////////////////////////////////////////////////////////////////////////
