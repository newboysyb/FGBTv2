    ...
    public class postreply : PageBase
    {
        ...
        protected override void ShowPage()
        {
            ...
            if (ispost)
            {
                ...
                postmessage = postmessage.Replace("[quoteinside]", "[quote]").Replace("[/quoteinside]", "[/quote]");

                //【END BT修改】
                //////////////////////////////////////////////////////////////////////////
                //////////////////////////////////////////////////////////////////////////

                //获取被回复帖子的作者uid //此处的postid还是被回复人的postid
                int replyUserid = -1;
                int layer = 1, parentid = 0, rootparentid = 0, replied = 0;

                if(postid > 0)
                {
                    PostInfo replypost = Posts.GetPostInfo(topicid, postid);
                    if (replypost == null)
                    {
                        AddErrLine("回复到的帖子ID无效! 帖子不存在 或 您的浏览器提交了错误信息，请关闭异常的自动填表功能");
                        return;
                    }
                    if(replypost.Tid != topicid)
                    {
                        AddErrLine("回复到的主题ID无效! 主题不存在 或 您的浏览器提交了错误信息，请关闭异常的自动填表功能");
                        return;
                    }
                    replyUserid = replypost.Posterid;

                                    //【BT修改】记录所回复的帖子id，以及根回复贴id
                    parentid = replypost.Pid;
                    if (replypost.RootParentid > 0) rootparentid = replypost.RootParentid;
                    else if (replypost.Parentid > 0) rootparentid = replypost.Parentid;      //这个是不必要的
                    else rootparentid = parentid;
                    replied = replypost.Replied;

                    int replytocheck = Posts.CheckReplyToPoster(postid, postmessage, posttitle);
                    if (replytocheck < 0)
                    {
                        AddErrLine("回复到的帖子ID与帖子内容不匹配! 帖子不存在 或 您的浏览器提交了错误信息，请关闭异常的自动填表功能");
                        //AddErrLine("回复到的帖子ID与帖子内容不匹配! 帖子不存在 或 您的浏览器提交了错误信息，请关闭异常的自动填表功能" + replytocheck + "|" + posttitle);
                        return;
                    }
                    else if (replytocheck == 2)
                    {
                        //AddErrLine(replytocheck + "|" + posttitle);
                        posttitle = posttitle.Replace("#" + postid.ToString() + "#", "");
                        //AddErrLine("---" + replytocheck + "|" + posttitle);
                    }
                }
                else
                {
                    //如果没有回复给特定帖子，则相当于回复给主题作者
                    replyUserid = topic.Posterid;
                }
                
                //创建帖子内容
                postinfo = CreatePostInfo(postmessage, parentid, rootparentid);

                //从这里开始：【postid】变为发表回复人的postid
                postid = postinfo.Pid;  
                if (IsErr())
                    return;
        ...
        /// <param name="postmessage"></param>
        /// <returns></returns>
        public PostInfo CreatePostInfo(string postmessage, int parentid, int rootparentid)
        {
            PostInfo postinfo = new PostInfo();
            postinfo.Fid = forumid;
            postinfo.Tid = topicid;
            
            //【BT修改】parentid作为“回复xx”功能的记录变量，此处postid仍为被回复人的postid，尚未被修改
            ...
            //////////////////////////////////////////////////////////////////////////
            //////////////////////////////////////////////////////////////////////////
            //【BT修改】回复帖子不审核，1of2 sessionajaxpage.cs
            ...
            if (postid != -1)
            {
                ...
                if (!string.IsNullOrEmpty(DNTRequest.GetString("quote")))
                {
                    ...
                    else
                    {
                        //////////////////////////////////////////////////////////////////////////
                        //////////////////////////////////////////////////////////////////////////
                        //【BT修改】 检查是否包含部分隐藏内容如[hide=xxxx][/hide]格式
                        postinfo.Message = System.Text.RegularExpressions.Regex.Replace(postinfo.Message, @"\[hide=\d+\].+\[/hide\]", "***这是隐藏内容***",
                            System.Text.RegularExpressions.RegexOptions.IgnoreCase);
                       
                        //【BT修改】 引用格式修正
                        string quotemessage = UBB.ClearAttachUBB(postinfo.Message);

                        //string quotemessage = UBB.ClearAttachUBB(Utils.GetSubString(postinfo.Message, 2000, "......"));

                        //quotemessage = UBB.ClearAttachUBB(Utils.GetSubString(postinfo.Message, 200, "......"));

                        //if ((quotemessage.Length - quotemessage.Replace("[quote]", "").Length) / 7 > (quotemessage.Length - quotemessage.Replace("[/quote]", "").Length) / 8)
                        //{
                        //    quotemessage = quotemessage + "[/quote]";
                        //}
                        //quotemessage = quotemessage.Replace("[quote]", "[quoteinside]").Replace("[/quote]", "[/quoteinside]");

                        // 修改为相对地址，取代原有的url={5}...Utils.GetRootUrl(forumpath)
                        message = string.Format("[quote][color=#999999]{1} 发表于 {2} [/color][url=showtopic.aspx?topicid={3}&postid={4}#{4}][img]images/common/back.gif[/img][/url]\r\n{0}[/size][/quote]"
                      , quotemessage, postinfo.Poster, postinfo.Postdatetime, topicid, postid);
