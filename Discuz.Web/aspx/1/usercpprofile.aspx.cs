...
namespace Discuz.Web
{
    ...
    public class usercpprofile : UserCpPage
    {
        ...
        ///// 是否检测旧密码
        ///// </summary>
        //public bool donotcheckpassword = false;
        ...
        protected override void ShowPage()
        {
            ...
            if (user.Email.ToLower().IndexOf("@ihome.buaa.edu.cn") > 0 || user.Email.ToLower().IndexOf("@yahoo.cn") > 0 || user.Email.ToLower().IndexOf("@yahoo.com.cn") > 0)
            {
                canchangeemail = true;
            }
            if (user.Email.ToLower().IndexOf("@ihome.buaa.edu.cn") > 0)
            {
                showemail = false;
            }
            ...
            if (DNTRequest.IsPost())
            {
                ...
                if (page_err == 0)
                {
                    ...
                    if (canchangeemail && userInfo.Email.ToLower().IndexOf("@ihome.buaa.edu.cn") < 0 && 
                        userInfo.Email.ToLower().IndexOf("@yahoo.cn") < 0 && userInfo.Email.ToLower().IndexOf("@yahoo.com.cn") < 0)
                    {
                        //只允许将含有ihome的邮箱更改为不含ihome的邮箱
                        if (userInfo.Email != DNTRequest.GetString("email2").Trim().ToLower())
                        {
                            AddErrLine("两次输入的Email地址不一致！");
                            return;
                        }
                    }
                    else if(canchangeemail)
                    {
                        ...
                        AddMsgLine("注意：您没有修改Email地址或输入的Email地址不符合要求，Emai地址没有被修改");
                    }
        ...
        private bool CheckModified(UserInfo oldUserInfo, UserInfo userInfo)
        {
            ...
            AddModLog(userInfo.Uid, "NickName", userInfo.Nickname, oldUserInfo.Nickname);
            AddModLog(userInfo.Uid, "Bio", userInfo.Bio, oldUserInfo.Bio);
            AddModLog(userInfo.Uid, "Signature", userInfo.Signature, oldUserInfo.Signature);
        ...
        private void AddModLog(int uid, string section, string newvalue, string originalvalue)
        {
            if(originalvalue != newvalue) Users.InsertUserFieldHistory(uid, userid, ipaddress, section, originalvalue, newvalue);
        }
