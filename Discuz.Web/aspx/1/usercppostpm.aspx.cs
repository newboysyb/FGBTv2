    ...
    public class usercppostpm : UserCpPage
    {
        ...
        private void CreatePM(int folder, int saveToSendBox, string url, string msg)
        {
            if (folder != 2)
            {
                ...
                if (touser.Groupid > 3 && touser.Groupid < 10 && user.Groupid > 3)
                {
                    AddErrLine("抱歉,对方的用户身份目前不能接收您的短消息");
                    return;
                }
                //当前用户身份校验
                if (user.Groupid > 3 && user.Groupid < 10 && touser.Groupid > 3)
                {
                    AddErrLine("抱歉,您当前的用户身份不能发送短消息给普通用户");
                    return;
                }
