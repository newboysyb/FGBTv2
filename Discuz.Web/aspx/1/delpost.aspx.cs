...
namespace Discuz.Web
{
    ...
    public class delpost : PageBase
    {
        ...
        protected override void ShowPage()
        {
            ...
            if (DNTRequest.GetQueryString("action") == "closesink" || forumid == 37)
            {
                pagetitle = string.Format("结束 {0}", post.Title);
                forumnav = ShowForumAspxRewrite(forum.Pathlist.Trim(), forumid, forumpageid);

                //权限校验，必须本人，必须跳蚤市场，必须是主题帖
                if (topic.Posterid != userid || topic.Fid != 37 || post.Layer != 0)
                {
                    AddErrLine("您无权关闭此主题");
                    return;
                }
                ...
                int rst = 0;
                rst += TopicAdmins.SetClose(topic.Tid.ToString(), 1);
                TopicAdmins.BumpTopics(topic.Tid.ToString(), -1);

                if (rst > 0)
                {
                    NoticeInfo notice = new NoticeInfo();
                    notice.Postdatetime = Utils.GetDateTime();
                    notice.Type = NoticeType.TopicAdmin;
                    notice.Poster = "系统";
                    notice.Posterid = 0;
                    notice.Uid = userid;
                    notice.Note = string.Format("您已成功结束跳蚤市场交易主题：{0}", topic.Title);

                    Notices.CreateNoticeInfo(notice);

                    AddMsgLine("关闭跳蚤市场交易主题成功, 返回板块");


                    //更新标题
                    topic = Topics.GetTopicInfo(topic.Tid);
                    if (topic.Title.Length > 200) topic.Title = topic.Title.Substring(0, 190);
                    topic.Title = topic.Title + " [已结束]";
                    Topics.UpdateTopic(topic);
                }

                SetUrl(rst > 0 ? base.ShowForumAspxRewrite(post.Fid, 0) : Urls.ShowTopicAspxRewrite(post.Tid, 1));
                SetMetaRefresh(3);
            ...
            else
            {
                ...
                else
                {
                    ...
                    if (PrivateBT.Forum2Type(forum.Fid) > 0)
                        Discuz.Forum.Topics.UpdateTopicReplyCount(topicid, 1);
                    else
                        Discuz.Forum.Topics.UpdateTopicReplyCount(topicid, -1);
        ...
        private bool CheckPermission(PostInfo post, int opinion)
        {
            ...
            //////////////////////////////////////////////////////////////////////////
            ////////////////////////////////////////////////////////////////////////// 
            //【BT修改】如果删除的帖子是种子发布区的主贴
            
            if (PrivateBT.Forum2Type(post.Fid) > 0 && post.Layer == 0)
            {
                HttpContext.Current.Response.Redirect(string.Format("{0}seedadmin.aspx?action=moderate&operat=seeddeleteself&forumid={1}&topicid={2}", forumpath, post.Fid, post.Tid));
                return false;
            }
