    ...
    public class setnewpassword : PageBase
    {
        ...
        protected override void ShowPage()
        {
            ...
            if ((!__userinfo.Authstr.Equals(Authstr)) || Convert.ToDateTime(__userinfo.Authtime) <  DateTime.Now.AddDays(-3))
            {
                ...
                //SetMetaRefresh(5);
                //SetShowBackLink(false);
                int errorcount = LoginLogs.UpdateLoginLog(DNTRequest.GetIP(), true);
                AddErrLine("验证码错误或已失效,新的验证码已经通过 Email 发送到您的信箱中,<BR />请在 3 天之内到论坛修改您的密码. 请勿刷新此页面。 <BR />验证码错误尝试第" + errorcount + "次，共5次机会");
                return;
            }
            ...
            if (DNTRequest.IsPost())
            {
                ...
                //if (!Utils.IsSafeSqlString(DNTRequest.GetString("newpassword")))
                //    AddErrLine("输入的密码中包括不允许使用的特殊字符.如以下字符： - ; , / ( ) [ ] } { % @ * ! '等");

                if (IsErr()) return;

                if (PTUsers.UpdateUserPassword(uid, DNTRequest.GetString("newpassword"), true))
                {
