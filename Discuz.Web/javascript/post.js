...
function showsmiles(index, typename, pageindex, seditorKey)
{
	...
	for (var id = minIndex; id <= maxIndex; id++) {
		...
		html += '<td valign="middle"><img style="cursor: pointer;" src="' + url + 'editor/images/smilies/' + s[id]['url'] + '" id="smilie_' + s[id]['code'] + '" alt="' + s[id]['code'] + '" onclick="' + clickevt + '" title="" border="0" height="20" width="20" /></td>';
		if (ci%colCount == 0) {
			html += '</tr><tr>'
		}
		ci ++;
...
function uploadAttach(curId, statusid, prefix) {
	...
	if(curId > 0) {
		...
		if(nextId == curId || in_array(statusid, [6, 8])) {
			...
			if(UPLOADFAILED > 0) {
				showDialog('附件上传完成bbbbbbb！成功 ' + UPLOADCOMPLETE + ' 个，失败 ' + UPLOADFAILED + ' 个:' + FAILEDATTACHS);
				FAILEDATTACHS = '';
			}
			UPLOADSTATUS = 2;
			for(var i = 0; i < AID - 1; i++) {
...
alert('a');
	...
	if(arr[0] == 'DISCUZUPLOAD' && arr[2] == 0) {
	   ...
	   alert(str);
		showDialog('上传失败aaaa:' + STATUSMSG[arr[2]]);
	}
}
