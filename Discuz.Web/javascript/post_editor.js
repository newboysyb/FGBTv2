...
var verctrl_post = '20140616';

function parseurlwithcode(originalmessage)
{
    var messagep = originalmessage;
    if (!fetchCheckbox('bbcodeoff') && allowbbcode) {
        messagep = messagep.replace(/\[code\]([\s\S]+?)\[\/code\]/ig, function ($1, $2) { return parsecode($2); });
    }
    messagep = parseurl(messagep);
        for (var i = 0; i <= DISCUZCODE['num']; i++) {
        messagep = messagep.replace("[\tDISCUZ_CODE_" + i + "\t]", html2bbcode(DISCUZCODE['html'][i]));
    }
    return messagep;
}
function validate(theform, previewpost) {
    if (typeof (titleEditor) != 'undefined' && titleEditor != null) {
        ...
        if (htmltitle != '') {
            $("title").value = htmltitle.replace(/<[^>]*>/ig, '').toString().substr(0, 255);
        }
    }
    //switchEditor(0);
    //switchEditor(1);
    ...
    var message = bbinsert && wysiwyg ? html2bbcode(getEditorContents()) : (!theform.parseurloff.checked ? parseurlwithcode(originalmessage) : originalmessage);
    ...
    if (in_array($('postsubmit').name, ['topicsubmit', 'editsubmit'])) {
        ...
        if ($("createlottery") || $("updatelottery")) {
            var pollcount = 0;
            var polls = document.getElementsByName("pollitemid");
            var polloptionids = document.getElementsByName("optionid");
            var displayorders = document.getElementsByName("displayorder");
            ...
            if (pollcount < 2) {
                floatalert("博彩项不得少于2个");
                $("PollItemname").value = "";
                return false;
            }

            if (pollcount > maxpolloptions) {
                floatalert("系统设置为博彩项不得多于" + maxpolloptions + "个");
                $("PollItemname").value = "";
                return false;
            }
        }
