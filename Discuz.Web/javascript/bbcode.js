...
function bbcode2html(str) {
    ...
    if (!fetchCheckbox('bbcodeoff') && allowbbcode) {
        str = clearcode(str);
        /*【BT修改】 AT功能*/
        str = str.replace(/\[atuser\](.*?)\[\/atuser\]/ig, '<a href="atuser:$1">@$1</a>');
        str = str.replace(/\[url=atuser:([^\s\[\"']+?)\]@([\s\S]+?)\[\/url\]/ig, '<a href="atuser:$1">@$2</a>');
        ...
        str = str.replace(/\[float=([^\[\<]+?)\]/ig, '<br style="clear: both"><span style="float: $1;">');
        /*str = str.replace(/\[quote]([\s\S]*?)\[\/quote\]\s?\s?/ig, '<div class="quote"><blockquote>$1</blockquote></div>\n');*/
        str = str.replace(/\[quote\]/ig, '<div class="quote"><blockquote>');
        str = str.replace(/\[\/quote\]/ig, '</blockquote></div>');
        re = /\[table(?:=(\d{1,4}%?)(?:,([\(\)%,#\w ]+))?)?\]\s*([\s\S]+?)\s*\[\/table\]/ig;
        for (i = 0; i < 4; i++) {
            str = str.replace(re, function ($1, $2, $3, $4) { return parsetable($2, $3, $4); });
        }
    ...
    str = str.replace(/<\/blockquote><\/div>$/ig, '</blockquote></div>&nbsp;&nbsp;<br />');
...
function html2bbcode(str) {
	...
	if(!fetchCheckbox('bbcodeoff') && allowbbcode) {
        ...
        /*【BT修改】【HTML2BBCCODE】 AT功能*/
        /*str = str.replace(/<a\s+?style="atuser"\s+?href="userinfo.aspx\?username=.*?">@(.*?)<\/a>/ig, "[atuser]$1[/atuser]");*/
		...
		str = str.replace(/<div[^>]*quote[^>]*><blockquote>/ig, "[quote]");
		str = str.replace(/<\/blockquote><\/div>/ig, "[/quote]");
	...
	str = str.replace(/\[\/quote\]$/ig, '[/quote]\r');
