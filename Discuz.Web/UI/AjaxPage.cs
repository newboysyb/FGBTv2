    ...
    public class AjaxPage : Page
    {
        ...
        public AjaxPage()
        {
            ...
            if (Utils.StrIsNullOrEmpty(DNTRequest.GetUrlReferrer()))
            {
                ...
                if (userInfo == null || DNTRequest.GetString("appid") != Utils.MD5(userInfo.Username + userInfo.Password + userInfo.Uid + TypeConverter.StrToInt(input[1].ToString())))
                    return;
            }
            ...
            if (Utils.InArray(type, "deleteattach,getattachlist,deletepostsbyuidanddays,deletepost," + 
                "ignorepost,passpost,deletetopic,ignoretopic,passtopic,passtoforum25topic,getimagelist,getblocklist," + 
                "getpagelist,forumtree,topictree,quickreply,report,getdebatepostpage,confirmbuyattach,getnewpms," + 
                "getnewnotifications,getajaxforums,checkuserextcredit,diggdebates,imagelist,seedautofill,imagewaterfall," +
                "todayfilesizelimit,checkin"))
            {
                //如果需要验证用户身份，跳转至继承了PageBase的页面
                try
                {
                    if (type == "imagewaterfall")
                        HttpContext.Current.Server.Transfer("sessionajax.aspx?t=" + type + "&reason=" + DNTRequest.GetString("reason") + "&fidlist=" + DNTRequest.GetString("fidlist"));
                    else
            ...
            {
                //【BT修改】安全增强，uid过滤
                int uidint = TypeConverter.StrToInt(DecodeUid(DNTRequest.GetString("input")).Split(',')[0], -1);
                if(uidint < 1) return;
        ...
        private void GetIpList()
        {
            ...
            catch //添加try语法, 以防止在并发情况下, 服务器端远程链接被关闭后出现应用程序 '警告'(事件查看器)
            {
                ...
                System.Web.HttpContext.Current.Response.Expires = 0;
                System.Web.HttpContext.Current.Response.Cache.SetNoStore();
                Discuz.Forum.PageBase.PageBase_ResponseEnd();
            }
        }
        ...
        public void GetRateLogList()
        {
            ...
            try
            {
                ...
                {
                    if (previoursUid != rate.Uid) uidCount++;
                    xmlnode.Append("<ratelog>");
                    xmlnode.AppendFormat("\r\n\t<rateid>{0}</rateid>", rate.Id);
                    xmlnode.AppendFormat("\r\n\t<uid>{0}</uid>", rate.Uid);
                    //评分也使用Avatar获取url地址，后台可配置是否使用静态地址还是动态地址
                    xmlnode.AppendFormat("\r\n\t<avatar>{0}</avatar>", Avatars.GetAvatarUrl(rate.Uid, AvatarSize.Small));
                    ...
                    ////////////////////////////////////////////////////////////////////////// 
                    //【BT修改】显示评分单位MB
                    if (rate.ExtCredits > 2) xmlnode.AppendFormat("\r\n\t<score>{0} MB</score>", rate.Score > 0 ? ("+" + rate.Score.ToString()) : rate.Score.ToString());
                    else xmlnode.AppendFormat("\r\n\t<score>{0}</score>", rate.Score > 0 ? ("+" + rate.Score.ToString()) : rate.Score.ToString());
                    //【END BT修改】
                    //////////////////////////////////////////////////////////////////////////
                    ////////////////////////////////////////////////////////////////////////// 
                ...
                //无需继续处理，因为评分次数和评分人数是不同的两个数据
                //int times_req = DNTRequest.GetFormInt("ratetimes", 0);
                //if (times_req != uidCount && times_req <= GeneralConfigs.GetConfig().DisplayRateCount)
                //{
                //     //此种需要更新评分数量的情况，什么时候会出现？？？增加了配置信息中的最大显示数量限制，否则当平分数大于最大显示数量时，将总出现错误
                //    Posts.UpdatePostRateTimes(DNTRequest.GetFormInt("tid", 0), DNTRequest.GetFormInt("pid", 0).ToString());
                //    PTLog.InsertSystemLog(PTLog.LogType.UserPassword, PTLog.LogStatus.Error, "RateTime UnMatch", 
                //        string.Format("TID:{0} -PID:{1} -TIMES_REQ:{2} -TIMES_UID:{3}",
                //        DNTRequest.GetFormInt("tid", 0), DNTRequest.GetFormInt("pid", 0), DNTRequest.GetFormInt("ratetimes", 0), uidCount));
                //}
            }
            catch //添加try语法, 以防止在并发情况下, 服务器端远程链接被关闭后出现应用程序 '警告'(事件查看器)
            {
