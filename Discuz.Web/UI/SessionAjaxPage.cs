...
namespace Discuz.Web.UI
{
    ...
    public class SessionAjaxPage : PageBase
    {
        protected override void ShowPage()
        {
            ...
            {
                if (DNTRequest.GetString("t") != "newpmnoticecount")
                {
                    return;
                }
            }
            ...
            switch (type)
            {
                ...
                case "newpmcount":
                    //新短消息数
                    try { GetNewPMCount(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET PMCOUNT", ex.ToString()); }
                    break;

                case "newnoticecount":
                    //新通知数
                    try { GetNewNoticeCount(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET NTCOUNT", ex.ToString()); }
                    break;

                case "newpmnoticecount":
                    //新短消息数+通知数
                    try { GetNewPMNoticeCount(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET PTCOUNT", ex.ToString()); }
                    break;

                case "forumtree":
                    //获得指定版块的子版块信息,以xml文件输出
                    try { GetForumTree();	 }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET FTREE", ex.ToString()); }
                    break;

                case "topictree":
                    //获得指定主题的回复信息,以xml文件输出
                    try { GetTopicTree(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET TTREE", ex.ToString()); }		
                    break;
                ...
                case "quickreply":
                    //快速回复主题
                    try { QuickReply();	 }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET QREPLY", ex.ToString()); }
                    break;

                case "report": 
                    //举报功能
                    try { Report(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET REPORT", ex.ToString()); } 
                    break;
                ...
                case "diggdebates":
                    try { DiggDebates(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET DIGDEB", ex.ToString()); } 
                    break;

                case "getdebatepostpage":
                    try { GetDebatePostPage(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET DPOSTP", ex.ToString()); } 
                    break;
                ...
                case "checkuserextcredit":
                    try { CheckUserExtCredit(DNTRequest.GetInt("aid", 0)); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET CCREDIT", ex.ToString()); } 
                    break;

                case "confirmbuyattach":
                    //购买指定附件
                    try { ConfirmBuyAttach(DNTRequest.GetInt("aid", 0)); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET CATTACH", ex.ToString()); } 
                    break;

                case "getnewpms":
                    try { GetNewPms(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET PMS", ex.ToString()); } 
                    //获得新短消息内容
                    break;

                case "getnewnotifications":
                    try { GetNewNotifications(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET NOTICE", ex.ToString()); } 
                    break;

                case "getajaxforums":
                    try { GetAjaxForumsJsonList(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET FROUM", ex.ToString()); } 
                    break;
                ...
                case "ignorepost":
                case "passtopic":
                case "passtoforum25topic":
                case "ignoretopic":
                case "deletetopic":
                    try { AuditPost(type, DNTRequest.GetString("reason")); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET DELTOPIC", ex.ToString()); } 
                    break; ;
                case "deletepostsbyuidanddays":
                    try { DeletePostsByUidAndDays(DNTRequest.GetInt("uid", 0), 7); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET DELPOST", ex.ToString()); } 
                    break;
                case "getattachlist":
                    try { GetAttachList(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET ATTACH", ex.ToString()); } 
                    break;
                case "deleteattach":
                    try { DeleteAttach(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET DELATT", ex.ToString()); } 
                    break;
                case "imagelist":
                    try { GetImageList(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "GET IMGLIST", ex.ToString()); } 
                    break;

                case "seedautofill": //种子自动填表
                    try { GetSeedAutoFill(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "SEEDAUTOFILL", ex.ToString()); } 
                    break;
                case "imagewaterfall": //图标瀑布流
                    try { GetImageWarterFall(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "IMAGE WATERFALL", ex.ToString()); }
                    break;
                case "todayfilesizelimit": //附件今日上传剩余尺寸
                    try { GetTodayFileSizeLimit(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "TFileSizeLimit", ex.ToString()); }
                    break;
                case "checkin": //附件今日上传剩余尺寸
                    try { CheckIn(); }
                    catch (System.Exception ex) { PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "CheckIn", ex.ToString()); }
                    break;
        ...
        private void GetTodayFileSizeLimit()
        {
            ResponseText(FormatBytes(usergroupinfo.Maxsizeperday - (userid > 0 ? Attachments.GetUploadFileSizeByuserid(userid) : 0)));
        }

        /// <summary>
        /// 签到
        /// </summary>
        private static object CheckInSyncObj = new object();
        /// <summary>
        /// 签到
        /// </summary>
        private void CheckIn()
        {
            try
            {
                ...
                int rank = -1;
                double addext = 0;
                float[] value = new float[8] { (float)0.0, (float)0.0, (float)0.0, (float)0.0, (float)0.0, (float)0.0, (float)0.0, (float)0.0 };
                if (newt.Date > oluserinfo.LastCheckInTime.Date)
                {
                    //间隔超过1天，重置数量
                    if ((newt.Date - oluserinfo.LastCheckInTime.Date).TotalDays > 1)
                    {
                        newc = 1;
                    }
                    else
                    {
                        newc = oluserinfo.LastCheckInCount + 1;
                    }


                    if (PTUsers.DoCheckIn(oluserinfo.Userid, oluserinfo.LastCheckInCount, oluserinfo.LastCheckInTime, newc, newt) > 0)
                    {

                        if (newc > 9) addext = 3;
                        else if (newc > 5) addext = 2;
                        else if (newc > 1) addext = 1;
                        checkinok = true;
                        ...
                        try
                        {
                            ...
                            lock (CheckInSyncObj)
                            {
                                rank = PTUsers.AddCheckInLog(oluserinfo.Userid, newt.Date, newt, newc);
                            }
                            double timecost = (DateTime.Now - st).TotalMilliseconds;

                            //对于高并发状态，增加获取和更新rank的延时
                            if (timecost > 2000) System.Threading.Thread.Sleep(PTTools.GetRandomInt(3000, 6000));
                            else if (timecost > 1600) System.Threading.Thread.Sleep(PTTools.GetRandomInt(2000, 5000));
                            else if (timecost > 1200) System.Threading.Thread.Sleep(PTTools.GetRandomInt(2000, 4000));
                            else if (timecost > 800) System.Threading.Thread.Sleep(PTTools.GetRandomInt(2000, 3000));
                            else if (timecost > 400) System.Threading.Thread.Sleep(PTTools.GetRandomInt(1000, 2000));
                            else if (timecost > 200) System.Threading.Thread.Sleep(PTTools.GetRandomInt(500, 1500));
                            else if (timecost > 100) System.Threading.Thread.Sleep(PTTools.GetRandomInt(0, 1000));

                            if (timecost > 100) PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Normal, "CheckIn", "TIMECOST(ms):" + timecost.ToString("0.00"));
                            
                            //if (DateTime.Now.Hour == 0 && DateTime.Now.Minute == 0 && DateTime.Now.Second < 2)
                            //{
                            //    System.Threading.Thread.Sleep(PTTools.GetRandomInt(3000, 5000));
                            //}
                            //else if (DateTime.Now.Hour == 0 && DateTime.Now.Minute == 0 && DateTime.Now.Second < 4)
                            //{
                            //    System.Threading.Thread.Sleep(PTTools.GetRandomInt(2000, 4000));
                            //}
                            //else if (DateTime.Now.Hour == 0 && DateTime.Now.Minute == 0 && DateTime.Now.Second < 6)
                            //{
                            //    System.Threading.Thread.Sleep(PTTools.GetRandomInt(1000, 3000));
                            //}

                            rank = PTUsers.GetCheckInRank(oluserinfo.Userid, newt.Date, newt, newc);
                        }
                        catch (System.Exception ex)
                        {
                            PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "CheckIn", "2." + ex.ToString());
                        }
                    }

                    if (rank < 101 && rank > 0) addext += 3;
                    if (addext > 0)
                    {
                        Discuz.Forum.Users.UpdateUserExtCredits(oluserinfo.Userid, 2, (float)addext);

                        value = new float[8] { (float)0.0, (float)addext, (float)0.0, (float)0.0, (float)0.0, (float)0.0, (float)0.0, (float)0.0 };
                    }
                }
                else checkinok = true;

                UserCredits.WriteUpdateUserExtCreditsCookies(value);
                string rtv = "";
                if (checkinok && rank > 0) rtv = string.Format("<a href=\"#\" class=\"fgbtcheckintoday\" title=\"已连续签到{0}天\">今天第" + rank + "个签到</a>", newc);
                else if (checkinok) rtv = string.Format("<a href=\"#\" class=\"fgbtcheckintoday\" title=\"已连续签到{0}天\">今天已签到</a>", newc);
                else rtv = string.Format("<a href=\"#\" class=\"fgbtcheckintoday\" title=\"已连续签到{0}天\">签到失败，请刷新后重试</a>", newc);
                ResponseText(rtv);
            }
            catch (System.Exception ex)
            {
                string rtv = string.Format("<a href=\"#\" class=\"fgbtcheckintoday\" title=\"\">签到失败，请刷新后重试</a>");
                ResponseText(rtv);
                PTLog.InsertSystemLog(PTLog.LogType.SessionAJAXPage, PTLog.LogStatus.Exception, "CheckIn", "1." + ex.ToString());
            }
            
        }

        private void GetImageWarterFall()
        {
            string rtv = "";

            int section = DNTRequest.GetInt("reason", -1);
            if (section > 10 || section < 1) section = 1;

            string fidlist = DNTRequest.GetString("fidlist");


            ForumHotConfigInfo forumhotconfiginfo = ForumHotConfigs.GetConfig();
            foreach (ForumHotItemInfo forumhotiteminfo in forumhotconfiginfo.ForumHotCollection)
            {
                if (forumhotiteminfo.Datatype == "pictures")
                {
                    rtv = ForumHots.HotImagesArray(forumhotiteminfo, usergroupid, ipaddress, section, fidlist);
                    break;
                }
            }
            ResponseText(rtv);
        ...
        private void GetSeedAutoFill()
        {
            if (!ispost)
            {
                System.Text.StringBuilder xmlnode = new System.Text.StringBuilder("<?xml version=\"1.0\" encoding=\"utf-8\"?><root><![CDATA[\n");
                xmlnode.Append("<div style=\"margin:10px;\"><b>自动标签（此功能正在测试中，如有问题可到意见投诉版反馈！）</b><br /> 请在此处粘贴符合通常标准的电影介绍，由未来花园自动寻找符合标签项目中的内容<br />注意：提交后将覆盖当前填写的内容！请仔细核并修改错误的内容<br />");
                xmlnode.Append("<form id=\"post_seedautofill\" action=\"tools/ajax.aspx?t=seedautofill&reason=movie\"><textarea id=\"seedautofill_area\" class=\"fgbt_seedautofill_area\" rows=\"20\" cols=\"50\" style=\"width:800px;height:400px;\"></textarea><br />");
                xmlnode.Append("<input id=\"seedautofill_text\" type=\"text\" style=\"display:none;\" value=\"\" />");
                xmlnode.Append("<input type=\"button\" onclick=\"do_seedautofill()\" value=\" 提交自动填写 \" />");
                xmlnode.Append("<input type=\"button\" onclick=\"hideWindow('seedautofillw')\" value=\" 取消 \" /></div>");
                xmlnode.Append("]]></root>");
                ResponseXML(xmlnode);
            }
            else
            {
                //ResponseText("OK! SEEDNAME:" + DNTRequest.GetString("seedname") + "OK! MESSAGE:" + DNTRequest.GetString("message"));
                string title = DNTRequest.GetString("seedname").Replace("Cfakepath", "");
                string message = PTTools.UnEscape(DNTRequest.GetString("message"));
                
                PT_SeedTagInfo s = new PT_SeedTagInfo();

                string tmp = "";
                int pos = 0;

                //获取英文名
                Regex r1 = new Regex(@"[^a-zA-Z0-9\.\-\s_]");
                s.Movie_ename = r1.Replace(title, "");
                s.Movie_ename = s.Movie_ename.Replace(".torrent", "");
                s.Movie_ename = s.Movie_ename.Replace(".MKV", "");
                s.Movie_ename = s.Movie_ename.Replace(".mkv", "");
                s.Movie_ename = s.Movie_ename.Replace(".MP4", "");
                s.Movie_ename = s.Movie_ename.Replace(".mp4", "");
                if (s.Movie_ename.Length > 1 && s.Movie_ename.Substring(0, 1) == ".") s.Movie_ename = s.Movie_ename.Substring(1);


                //获取年份
                //匹配标准命名的年份信息
                r1 = new Regex(@"\b(19\d{2}|200\d{1}|201\d{1})[\.\s](720p|BluRay|HDTV|HDRip|LIMITED|UNCUT)\b", RegexOptions.IgnoreCase);
                if (r1.IsMatch(title)) tmp = r1.Match(title).Value;
                else
                {
                    //标准匹配失败，匹配命名中的独立4位年份
                    r1 = new Regex(@"\b.?(19\d{2}|200\d{1}|201\d{1}).?\b");
                    if (r1.IsMatch(title)) tmp = r1.Match(title).Value;
                    else
                    {
                        //匹配介绍中的4位年份
                        r1 = new Regex(@"(19\d{2}|200\d{1}|201\d{1})");
                        if (r1.IsMatch(message)) tmp = r1.Match(message).Value;
                        else tmp = "0000";
                    }
                }
                r1 = new Regex(@"(19\d{2}|200\d{1}|201\d{1})");
                if (r1.IsMatch(tmp)) tmp = r1.Match(tmp).Value;
                else tmp = "0000";
                s.Movie_year = tmp;


                //获取国家地区
                r1 = new Regex(@"^.*(国\s*家|地\s*区|产\s*地).*$", RegexOptions.Multiline);
                if (r1.IsMatch(message))
                {
                    tmp = r1.Match(message).Value;
                    r1 = new Regex(@"(国\s*家|地\s*区|产\s*地)");
                    //获取最后一个匹配
                    MatchCollection matches = r1.Matches(tmp);
                    pos = 0;
                    foreach (Match match in matches)
                    {
                        int t = tmp.IndexOf(match.Value) + match.Value.Length;
                        if (t > pos) pos = t;
                    }
                    if (pos > 0) tmp = tmp.Substring(pos);
                    else tmp = "";

                    tmp = PTTools.TagStringFilter(tmp);

                    s.Movie_region = tmp;
                }
                else
                {
                    if (PTTools.StringContainsAny(title, "CHC")) s.Movie_region = "中国大陆";
                    s.Movie_region = "地区";
                }

                //获取片名
                //获取国家地区
                r1 = new Regex(@"^.*(译\s*名).*$", RegexOptions.Multiline);
                if (r1.IsMatch(message))
                {
                    tmp = r1.Match(message).Value;
                    r1 = new Regex(@"(译\s*名)");
                ...
                {
                    r1 = new Regex(@"^.*(片\s*名).*$", RegexOptions.Multiline);
                    if (r1.IsMatch(message))
                    {
                        tmp = r1.Match(message).Value;
                        r1 = new Regex(@"(片\s*名)");
                ...
                //获取电影类别
                r1 = new Regex(@"^.*(类\s*别|类\s*型).*$", RegexOptions.Multiline);
                if (r1.IsMatch(message))
                {
                    tmp = r1.Match(message).Value;
                    r1 = new Regex(@"(类\s*别|类\s*型)");
                    //获取最后一个匹配
                    MatchCollection matches = r1.Matches(tmp);
                    pos = 0;
                    foreach (Match match in matches)
                ...
                //获取imdb编号
                r1 = new Regex(@"\bimdb.com/title/tt\d{5,10}\b");
                if (r1.IsMatch(message))
                {
                    tmp = r1.Match(message).Value;
                    tmp = tmp.Replace("imdb.com/title/", "").Trim();

                    s.Movie_imdb = tmp;
                }
                else
                {
                    r1 = new Regex(@"tt\d{5,10}");
                    if (r1.IsMatch(message))
                    {
                        tmp = r1.Match(message).Value;
                        s.Movie_imdb = tmp;
                ...
                //获取电影语言
                r1 = new Regex(@"^.*(语\s*言).*$", RegexOptions.Multiline);
                if (r1.IsMatch(message))
                {
                    tmp = r1.Match(message).Value;
                    r1 = new Regex(@"(语\s*言)");
                ...
                if (title.ToLower().IndexOf("720p") >= 0) s.Movie_resolution = "720P";
                else if (title.ToLower().IndexOf("1080i") >= 0) s.Movie_resolution = "1080i";
                else if (title.ToLower().IndexOf("1080p") >= 0) s.Movie_resolution = "1080P";
                else if (title.ToLower().IndexOf("bdrip") >= 0) s.Movie_resolution = "BDRIP";
                else if (title.ToLower().IndexOf("hddverip") >= 0) s.Movie_resolution = "HDDVDRIP";
                else if (title.ToLower().IndexOf("dvdrip") >= 0) s.Movie_resolution = "DVDRIP";
                else if (title.ToLower().IndexOf("dvdrip") >= 0) s.Movie_resolution = "DVDRIP";
                else if (title.ToLower().IndexOf("dvdscr") >= 0) s.Movie_resolution = "DVDSCR";
                else if (title.ToLower().IndexOf("r5") >= 0) s.Movie_resolution = "R5";
                else if (title.ToLower().IndexOf("minisd") >= 0) s.Movie_resolution = "MiniSD";
                else if (title.ToLower().IndexOf("halfcd") >= 0) s.Movie_resolution = "HalfCD";
                else s.Movie_resolution = "";

                //字幕
                s.Movie_subtitle = "中文字幕";
                //if (title.ToLower().IndexOf("minisd") > -1) s.Movie_subtitle = "中文字幕";

                //分级
                if (s.Movie_type == "") s.Movie_rank = "N/A";
                else if (PTTools.StringContainsAny(s.Movie_type, "悬疑,罪案,犯罪,恐怖,惊悚,战争")) s.Movie_rank = "R";
                else if (PTTools.StringContainsAny(s.Movie_type, "动画")) s.Movie_rank = "PG";
                else s.Movie_rank = "PG-13";


                message = message.Replace("\n", "<br />"); //.Replace("·", " ")
                string rtv = string.Format("{{\"cname\":\"{0}\",\"ename\":\"{1}\",\"imdb\":\"{2}\",\"type\":\"{3}\",\"resolution\":\"{4}\",\"year\":\"{5}\",\"region\":\"{6}\",\"language\":\"{7}\",\"subtitle\":\"{8}\",\"rank\":\"{9}\",\"message\":\"{10}\"}}",
                s.Movie_cname,
                s.Movie_ename,
                s.Movie_imdb,
                s.Movie_type,
        ...
        private void QuickReply()
        {
            ...
            int postid = 0;
            string topictitle = "";
            int layer = 1, parentid = 0, rootparentid = 0, replied = 0;
            ...
            #region 校验回帖信息有效性，回复到帖子的ID必须与帖子内容中的回复到信息相同
            //获取回复帖ID
            ...
            if (replyPostid > 0 && DNTRequest.GetString("postreplynotice") == "true")
            {
                ...
                if (postinfo == null)
                {
                    ResponseXML(xmlnode.Append("<error>回复到的帖子ID无效! 帖子不存在 或 您的浏览器提交了错误信息，请关闭异常的自动填表功能</error>"));
                    return;
                }
                if (topicid != postinfo.Tid)
                {
                    ResponseXML(xmlnode.Append("<error>回复到的主题ID无效! 主题不存在 或 您的浏览器提交了错误信息，请关闭异常的自动填表功能</error>"));
                    return;
                }
                replyuserid = postinfo.Posterid;//回复某楼作者，用于下面发送通知时使用
                
                //【BT修改】记录所回复的帖子id，以及根回复贴id
                parentid = postinfo.Pid;
                if (postinfo.RootParentid > 0) rootparentid = postinfo.RootParentid;
                else if (postinfo.Parentid > 0) rootparentid = postinfo.Parentid;      //这个是不必要的
                else rootparentid = parentid;
                replied = postinfo.Replied;
                

                int replytocheck = Posts.CheckReplyToPoster(replyPostid, postmessage, posttitle);
                if (replytocheck < 0)
                {
                    ResponseXML(xmlnode.Append("<error>回复到的帖子ID与帖子内容不匹配! 帖子不存在 或 您的浏览器提交了错误信息，请关闭异常的自动填表功能</error>"));
                    return;
                }
                else if (replytocheck == 2)
                {
                    posttitle = posttitle.Replace("#" + replyPostid.ToString() + "#", "");
                }
            ...
            #region 生成帖子信息，由于之前postinfo被用于获取被回复贴信息，此处必须全部赋值
            //是否解析HTML
            ...
            postinfo.RootParentid = rootparentid;
            ...
            //////////////////////////////////////////////////////////////////////////
            //////////////////////////////////////////////////////////////////////////
            //【BT修改】回复帖子不审核，1of2 postreply.cs
            ...
            //更新被回复贴状态
            if (replied == 0) Posts.UpdatePostSetReplied(parentid, topicid);
            ...
            if(topic.SeedId > 0)
                Topics.UpdateTopicReplyCount(topicid, 1);
            else
                Topics.UpdateTopicReplyCount(topicid, -1);
        ...
        private void Report()
        {
            ...
            else
            {
                if (!Utils.StrIsNullOrEmpty(reportUrl))
                {
                    ...
                    //用户输入的举报理由，进行html编码
                    string reportmessage_encodedfull =   Utils.HtmlEncode(reportmessage);
                    string reportmessage_encodedshort = reportmessage.Length > 30 ? Utils.HtmlEncode(reportmessage.Substring(0,30)): Utils.HtmlEncode(reportmessage);
                    
                    //【BT修改】此处之前无安全校验？？？ reporturl 直接被粘贴到短消息中
                    string reportUrl_encoded = Uri.EscapeDataString(reportUrl);
                    int report_pid = -1;
                    int report_tid = PTTools.CheckReportUrlFormat(reportUrl, ref report_pid);
                    if (report_pid < 1 || report_tid < 1)
                    {
                        PTLog.InsertSystemLog(PTLog.LogType.ReportFormat, PTLog.LogStatus.Error, "举报链接格式错误",
                            string.Format("UID:{0}--IP:{1}--REPORTURL:{2}", userid, ipaddress, reportUrl_encoded));
                        ResponseXML(xmlnode);
                        return;
                    }

                    //生成举报信息
                    string message = string.Format(@"下面的链接地址被举报,<br /><a href='{0}' target='_blank'>http://buaabt.cn/{0}</a><br />请检查<br/>举报理由：{1}", reportUrl, reportmessage_encodedfull);
                    ...
                    //给举报者发送举报成功消息
                    message = string.Format(@"您举报了以下链接地址,<br /><a href='{0}' target='_blank'>http://buaabt.cn/{0}</a><br /><br />举报理由：{1}<br /><br />我们会尽快核实、处理或回复您的举报信息，感谢您的支持！", reportUrl, Utils.HtmlEncode(reportmessage));
                    pm.Message = message;
                    pm.Subject = "已经收到您的举报信息";
                    pm.Msgto = username;
                    pm.Msgtoid = userid;
                    pm.Msgfrom = "系统";
                    pm.Msgfromid = 0;
        ...
        private string GetCheckUserExtCreditJson(AttachmentInfo attachmentinfo)
        {
            ...
            if (userid > 0 && attachmentinfo.Aid > 0)
            {
                ...
                decimal extcredit = Users.GetUserExtCreditsByUserid(userid, Scoresets.GetTopicAttachCreditsTrans());
        ...
        public void GetNewPMCount()
        {
            if (oluserinfo.Olid > 0 && oluserinfo.Userid > 0)
            {
                HttpContext.Current.Response.ExpiresAbsolute = DateTime.Now.AddSeconds(-1);
                HttpContext.Current.Response.Expires = -1;
                HttpContext.Current.Response.Clear();
                HttpContext.Current.Response.Write("<PMCOUNT>" + oluserinfo.Newpms);
                PageBase_ResponseEnd();
            }
            else
            {
        ...
        public void GetNewPMNoticeCount()
        {
            if (oluserinfo.Olid > 0 && oluserinfo.Userid > 0)
            {
                HttpContext.Current.Response.ExpiresAbsolute = DateTime.Now.AddSeconds(-1);
                HttpContext.Current.Response.Expires = -1;
                HttpContext.Current.Response.Clear();
                HttpContext.Current.Response.Write("<PMCOUNT><" + oluserinfo.Newpms + "><NOTICECOUNT><" + oluserinfo.Newnotices + ">");
                PageBase_ResponseEnd();
            }
            else
            {
                //暂时无效，因为异常用户pagebase就返回了
                HttpContext.Current.Response.ExpiresAbsolute = DateTime.Now.AddSeconds(-1);
                HttpContext.Current.Response.Expires = -1;
                HttpContext.Current.Response.Clear();
                HttpContext.Current.Response.Write("<PMCOUNT><-1099><NOTICECOUNT><-1099>");
                PageBase_ResponseEnd();
            }

        }
        public void GetNewNoticeCount()
        {
            if (oluserinfo.Olid > 0 && oluserinfo.Userid > 0)
            {
                HttpContext.Current.Response.ExpiresAbsolute = DateTime.Now.AddSeconds(-1);
                HttpContext.Current.Response.Expires = -1;
                HttpContext.Current.Response.Clear();
                HttpContext.Current.Response.Write("<NOTICECOUNT>" + oluserinfo.Newnotices);
                PageBase_ResponseEnd();
            }
            else
            {
        ...
        public void AuditPost(string type, string reason)
        {
            ...
            if (usergroupinfo.Radminid > 0)
            {
                ...
                //判断版主是否有权限管理主题//【BT修改】通过到品茶亭功能
                if (usergroupinfo.Radminid == 3 && "passtopic,passtoforum25topic,ignoretopic,deletetopic".Contains(type) && !Topics.GetModTopicCountByTidList(username, tid))
                    return;
                ...
                //判断版主是否有权限管理帖子,passtoforum25topic//【BT修改】通过到品茶亭功能
                ...
                if (("passtopic,passtoforum25topic,ignoretopic,deletetopic".Contains(type) && !CreateNoticeInfo(type, tid, reason)) ||
                ...
                switch (type)
                {
                    ...
                    case "passtoforum25topic":
                        ...
                        try
                        {
                            fid = Topics.GetTopicInfo(TypeConverter.StrToInt(tid.Split(',')[0], -1)).Fid;
                        }
                        catch (System.Exception ex)
                        {
                            ex.ToString();
                        ...
                        TopicAdmins.MoveTopics(tid, 25, fid, false, 0);
                        //Topics.PassAuditNewTopic(tableId.ToString(), "", tid, "", "25");
                        Topics.PassAuditNewTopic(tableId.ToString(), "", tid, "", "");
                        
                        //重新统计版面主题数的功能有问题，导致清零？
        ...
        private bool CreateNoticeInfo(string type, string idList, string reason)
        {
            ...
            //【BT修改】通过到品茶亭功能
            if ("passtopic,passtoforum25topic,ignoretopic,deletetopic".Contains(type))
            {
                foreach (string tid in idList.Split(','))
                {
                    ...
                    if (usergroupinfo.Reasonpm == 3 || reason != "")
                    {
                        ...
                        //【BT修改】通过到品茶亭功能
                        if (type == "passtoforum25topic")
                            message += string.Format("<a href='{0}' target='_blank'>{1}</a> 已经被转移到品茶亭并通过审核！ &nbsp; <a href='{0}' target='_blank'>查看 &rsaquo;</a> ",
                                Urls.ShowTopicAspxRewrite(TypeConverter.StrToInt(tid), 0), topic.Title);
