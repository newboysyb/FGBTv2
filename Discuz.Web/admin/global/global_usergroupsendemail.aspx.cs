    ...
    public partial class usergroupsendemail : AdminPage
    {
        ...
        private void BatchSendEmail_Click(object sender, EventArgs e)
        {
            ...
            if (this.CheckCookie())
            {
                ...
                //        foreach(DataRow dr in dt.Rows)
                //        {
                //            if (dr["email"] != null && dr["email"].ToString().Trim().ToLower() != "null" && dr["email"].ToString().Trim().ToLower() != "")
                //            {
                //                string emailadd = dr["email"].ToString().Trim().ToLower();
                //                if (emailadd.IndexOf("ihome.buaa.edu.cn") > -1) continue;
        ...
        {
            base.RegisterStartupScript("", "<script>alert('此功能因为消耗大量系统资源，已经被关闭');</script>");
            return;
