    ...
    public partial class forumtree : UserControl
    {
        ...
        public void LoadForumTree()
        {
            ...
            else //广告添加或编辑时显示
            {
                ...
                if ((this.SelectForumStr.IndexOf("登陆页面") >= 0) && (this.SelectForumStr.IndexOf("全部") < 0))
                {
                    sb.Append("<tr><td class=treetd> " + T_rootpic + "<img class=treeimg src=../images/htm.gif > <input class=\"input1\" type=checkbox id=\"" + this.ClientID + "\" name=\"" + this.ClientID + "\" value=\"登陆页面\"   checked> 登陆页面</td></tr>");
                }
                else
                {
                    sb.Append("<tr><td class=treetd> " + T_rootpic + "<img class=treeimg src=../images/htm.gif > <input class=\"input1\" type=checkbox id=\"" + this.ClientID + "\" name=\"" + this.ClientID + "\" value=\"登陆页面\"   > 登陆页面</td></tr>");
                }

                if ((this.SelectForumStr.IndexOf("注册页面") >= 0) && (this.SelectForumStr.IndexOf("全部") < 0))
                {
                    sb.Append("<tr><td class=treetd> " + T_rootpic + "<img class=treeimg src=../images/htm.gif > <input class=\"input1\" type=checkbox id=\"" + this.ClientID + "\" name=\"" + this.ClientID + "\" value=\"注册页面\"   checked> 注册页面</td></tr>");
                }
                else
                {
                    sb.Append("<tr><td class=treetd> " + T_rootpic + "<img class=treeimg src=../images/htm.gif > <input class=\"input1\" type=checkbox id=\"" + this.ClientID + "\" name=\"" + this.ClientID + "\" value=\"注册页面\"   > 注册页面</td></tr>");
                }
