...
namespace Discuz.Data
{
    public class Forums
    {
        ...
        {
            List<ForumHottopicsList> forumhottopicslist = new List<ForumHottopicsList>();
            DataTable dt = DatabaseProvider.GetInstance().GetForumHotTopicsList();

            ForumHottopicsList forumhottopicdata = new ForumHottopicsList();
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ForumHottopics topicinfo = new ForumHottopics();

                    int fid = TypeConverter.StrToInt(dr["fid"].ToString(), 0);
                    int tid = TypeConverter.StrToInt(dr["tid"].ToString(), 0);
                    string title = dr["title"].ToString();
                    string typename = dr["name"].ToString().ToLower().Trim();

                    topicinfo.Tid = tid;
                    topicinfo.TopicTitle = (typename.ToLower() != "null" && typename != "") ? ("[" + typename + "] " + title) : title;

                    if (fid > 0 && tid > 0)
                    {
                        if (fid != lastfid && lastfid != -1)
                        {
                            forumhottopicslist.Add(forumhottopicdata);
                            forumhottopicdata = new ForumHottopicsList();
                            forumhottopicdata.Fid = fid;
                        }
                        else if (lastfid == -1) forumhottopicdata.Fid = fid;

                        forumhottopicdata.HotTopicList.Add(topicinfo);
                        lastfid = fid;
                    }
                }
                forumhottopicslist.Add(forumhottopicdata);
        ...
        public static List<ForumInfo> GetForumList()
        {
            ...
            if (dt.Rows.Count > 0)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    ...
                    //////////////////////////////////////////////////////////////////////////
                    //【BT修改】增加rulesubb、rulesishtml、descriptionubb、descriptionishtml

                    forum.RulesUBB = dr["rulesubb"].ToString();
                    forum.RulesIsHTML = TypeConverter.ObjectToInt(dr["rulesishtml"]);
                    forum.DescriptionUBB = dr["descriptionubb"].ToString();
                    forum.DescriptionIsHTML = TypeConverter.ObjectToInt(dr["descriptionishtml"]);
