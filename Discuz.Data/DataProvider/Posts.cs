...
namespace Discuz.Data
{
    public class Posts
    {
        ...
        {
            return UpdatePost(postInfo, postTableId, -1, "");
        }
        /// <summary>
        /// 更新指定帖子信息，保留历史信息
        /// </summary>
        /// <param name="postInfo">帖子信息</param>
        /// <returns>更新数量</returns>
        public static int UpdatePost(PostInfo postInfo, string postTableId, int uid, string ip)
        {            
            int result = DatabaseProvider.GetInstance().UpdatePost(postInfo, postTableId, uid, ip);
            //更新Cache缓存中的帖子信息
        ...
        private static PostInfo LoadSinglePostInfo(IDataReader reader)
        {
            ...
            postInfo.RootParentid = TypeConverter.ObjectToInt(reader["rootparentid"]);
            postInfo.Parentid = TypeConverter.ObjectToInt(reader["parentid"]);
            postInfo.Replied = TypeConverter.ObjectToInt(reader["replied"]);
