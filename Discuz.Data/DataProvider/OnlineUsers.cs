...
namespace Discuz.Data
{
    public class OnlineUsers
    {
        ...
        public static OnlineUserInfo GetOnlineUserByRkey(int userid, string rkey)
        {
            OnlineUserInfo oluser = null;
            //if (appDBCache)
            //  oluser = IOnlineUserService.GetOnlineUserByRkey(userid, rkey);
            //else
            {
                DataTable dt = DatabaseProvider.GetInstance().GetOnlineUserByRKey(userid, rkey);
        ...
        private static OnlineUserInfo LoadSingleOnlineUser(IDataReader reader)
        {
            ...
            info.RKey = reader["rkey"].ToString();
            info.RkeyExpire = reader["rkeyexpire"].ToString();
            info.RkeyExpireTime = TypeConverter.ObjectToDateTime(reader["rkeyexpiretime"].ToString());
            ...
            info.LastCheckInTime = TypeConverter.ObjectToDateTime(reader["lastcheckintime"]);
            info.LastCheckInCount = TypeConverter.ObjectToInt(reader["lastcheckincount"]);

            info.Forumid = TypeConverter.ObjectToInt(reader["forumid"]);
            if (reader["forumname"] != DBNull.Value) info.Forumname = reader["forumname"].ToString();

            info.Titleid = TypeConverter.ObjectToInt(reader["titleid"]);
            if (reader["title"] != DBNull.Value) info.Title = reader["title"].ToString();

            info.Verifycode = reader["verifycode"].ToString();
            if (reader["newpms"] != DBNull.Value) info.Newpms = Int16.Parse(reader["newpms"].ToString());

            if (reader["newnotices"] != DBNull.Value) info.Newnotices = Int16.Parse(reader["newnotices"].ToString());
            ...
            info.PMSound = TypeConverter.ObjectToInt(reader["pmsound"], 0);
        ...
        private static OnlineUserInfo LoadSingleOnlineUser(DataRow dr)
        {
            ...
            //【BT修改】小于零时，直接赋值为-1，减少切换期问题
            info.Userid = TypeConverter.ObjectToInt(dr["userid"]) < 1 ? -1 : TypeConverter.ObjectToInt(dr["userid"]);

            info.RKey = dr["rkey"].ToString();
            info.RkeyExpire = dr["rkeyexpire"].ToString();
            info.RkeyExpireTime = TypeConverter.ObjectToDateTime(dr["rkeyexpiretime"].ToString());
            ...
            info.LastCheckInTime = TypeConverter.ObjectToDateTime(dr["lastcheckintime"]);
            info.LastCheckInCount = TypeConverter.ObjectToInt(dr["lastcheckincount"]);

            info.Forumid = TypeConverter.ObjectToInt(dr["forumid"]);

            if (dr["forumname"] != DBNull.Value) info.Forumname = dr["forumname"].ToString();

            info.Titleid = TypeConverter.ObjectToInt(dr["titleid"]);
            if (dr["title"] != DBNull.Value) info.Title = dr["title"].ToString();

            info.Verifycode = dr["verifycode"].ToString();
            if (dr["newpms"] != DBNull.Value) info.Newpms = Int16.Parse(dr["newpms"].ToString());

            if (dr["newnotices"] != DBNull.Value) info.Newnotices = Int16.Parse(dr["newnotices"].ToString());
            ...
            info.PMSound = TypeConverter.ObjectToInt(dr["pmsound"], 0);
