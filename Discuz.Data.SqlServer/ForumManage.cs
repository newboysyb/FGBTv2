...
namespace Discuz.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
            ...
            using (SqlTransaction trans = conn.BeginTransaction())
            {
                try
                {
                    DbParameter[] parms = {
                        ...
                        DbHelper.MakeInParam("@rulesubb", (DbType)SqlDbType.NText, 0, forumInfo.RulesUBB),
                        DbHelper.MakeInParam("@rulesishtml", (DbType)SqlDbType.Int, 4, forumInfo.RulesIsHTML),
                        DbHelper.MakeInParam("@descriptionubb", (DbType)SqlDbType.NText, 0, forumInfo.DescriptionUBB),
                        DbHelper.MakeInParam("@descriptionishtml", (DbType)SqlDbType.Int, 4, forumInfo.DescriptionIsHTML),
        ...
        {
            DataTable dt = PTTools.GetChar20TableFromString(extList);
            DbParameter[] parms = 
			{
            ...
            ((SqlParameter)parms[0]).TypeName = "fgbtChar20Table";

            string commandText = string.Format("select count(1) from [{0}myattachments] where [extname] IN (SELECT [Char20Value] FROM @TVP) and [UID]= @uid",
                                                BaseConfigs.GetTablePrefix);
        ...
        public IDataReader GetTopicBonusLogs(int tid, string postTableId)
        {
            ...
            string commandText = string.Format("SELECT b.[tid],[authorid],[answerid],[answername],b.[pid],[extid],[bonus],[isbest],[message] FROM [{0}bonuslog] AS b LEFT JOIN [{0}posts{1}] AS p ON b.[pid]=p.[pid] WHERE b.[tid]= @tid AND [bonus]<>0 ORDER BY [isbest] DESC",
        ...
        public int GetForumsLastPostTid(string fidList)
        {
            ...
            string commandText = string.Format("SELECT [tid] FROM [{0}topics] WHERE [lastpostid] = (SELECT MAX([lastpostid]) FROM [{0}topics] WHERE [fid] IN (SELECT [IntValue] FROM @TVP) AND [displayorder]>-1 AND [closed]=0)",
                                        BaseConfigs.GetTablePrefix,
                                        fidList);
            return TypeConverter.ObjectToInt(DbHelper.ExecuteScalar(CommandType.Text, commandText, parms));
        }
