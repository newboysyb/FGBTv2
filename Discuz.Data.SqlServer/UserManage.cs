    ...
    public partial class DataProvider : IDataProvider
    {
        ...
        public void AddMedalslog(int adminId, string adminName, string ip, string userName, int uid, string actions, int medals, string reason)
        {
            DbParameter[] parms = 
			{
                ...
                DbHelper.MakeInParam("@ip", (DbType)SqlDbType.NVarChar,45, ip),
        ...
        public DataTable GetUserEmailByGroupid(string groupIdList)
        {
            ...
            //                                    BaseConfigs.GetTablePrefix,
            //                                    groupIdList);
            string commandText = string.Format("SELECT [username],[email] From [{0}users] WHERE [groupid] IN({1})",
                                    BaseConfigs.GetTablePrefix,
                                    groupIdList);
            return DbHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0];
        }
        ...
        {
            DataTable dt = PTTools.GetIntTableFromString(fidList);
            DbParameter[] parms = {
            ...
            string commandText = string.Format("DELETE FROM [{0}favorites] WHERE [tid] IN (SELECT [IntValue] FROM @TVP) AND [uid] = @uid AND [typeid]=@typeid",
                                                BaseConfigs.GetTablePrefix,
                                                fidList);
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        /// <returns></returns>
        public DataTable GetOnlineUserByRKey(int userId, string rkey)
        {
            DbParameter[] parms = { 
                                        DbHelper.MakeInParam("@userid", (DbType)SqlDbType.Int, 4, userId),
                                        DbHelper.MakeInParam("@rkey", (DbType)SqlDbType.Char, 10, rkey)
                                    };
            return DbHelper.ExecuteDataset(CommandType.StoredProcedure, string.Format("{0}getonlineuserbyrkey", BaseConfigs.GetTablePrefix), parms).Tables[0];
        }
        ...
        public int DeletePrivateMessages(int userId, string pmIdList, string ipdel)
        {
            DataTable dt = PTTools.GetIntTableFromString(pmIdList);
            if (dt.Rows.Count < 1) return -1;
            DbParameter[] parms = {
                                      new SqlParameter("@TVP", dt),
									  DbHelper.MakeInParam("@userid", (DbType)SqlDbType.Int,4, userId),
                                      DbHelper.MakeInParam("@ipdel", (DbType)SqlDbType.VarChar, 45, ipdel),
                                      DbHelper.MakeInParam("@deldatetime", (DbType)SqlDbType.DateTime, 8, DateTime.Now),
			                      };
            ((SqlParameter)parms[0]).SqlDbType = SqlDbType.Structured;
            ((SqlParameter)parms[0]).TypeName = "fgbtIntTable";

            //////////////////////////////////////////////////////////////////////////
            //【BT修改】安全增强，msgtoid必须是folder=0，msgfromid必须是folder=1或者2的情况下
            //string commandText = string.Format("DELETE FROM [{0}pms] WHERE [pmid] IN (SELECT [IntValue] FROM @TVP) AND (([msgtoid] = @userid AND [folder] = 0) OR ([msgfromid] = @userid AND [folder] = 1) OR ([msgfromid] = @userid AND [folder] = 2))",
            //                                    BaseConfigs.GetTablePrefix);
            //【END BT修改】
            //////////////////////////////////////////////////////////////////////////
            return DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, "dnt_deletepm", parms);
        }
        ...
        public int GetNewPMCount(int userId)
        {
            ...
            string commandText = string.Format("SELECT COUNT([pmid]) AS [pmcount] FROM [{0}pms] WHERE [new] = 1 AND [folder] = 0 AND [msgtoid] = @userid",
                                                BaseConfigs.GetTablePrefix);
            return TypeConverter.ObjectToInt(DbHelper.ExecuteScalar(CommandType.Text, commandText, parms));
        }
        ...
        public int DeletePrivateMessage(int userId, int pmId)
        {
            ...
            //////////////////////////////////////////////////////////////////////////
            //【BT修改】安全增强，msgtoid必须是folder=0，msgfromid必须是folder=1或者2的情况下
            string commandText = string.Format("DELETE FROM [{0}pms] WHERE [pmid]=@pmid AND (([msgtoid] = @userid AND [folder] = 0) OR ([msgfromid] = @userid AND [folder] = 1) OR ([msgfromid] = @userid AND [folder] = 2))",
                                                BaseConfigs.GetTablePrefix);
            //【END BT修改】
            //////////////////////////////////////////////////////////////////////////
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        ...
        public DataTable GetUserGroupRateRange(int groupId)
        {
            ...
            string commandText = string.Format("SELECT TOP 1 [raterange] FROM [{0}usergroups] WHERE [groupid] = @groupId",
                                                BaseConfigs.GetTablePrefix);
            return DbHelper.ExecuteDataset(CommandType.Text, commandText, parms).Tables[0];
        }
        ...
        public IDataReader GetUserTodayRate(int uid)
        {
            ...
            string commandText = string.Format("SELECT [extcredits], SUM(ABS([score])) AS [todayrate] FROM [{0}ratelog] WHERE DATEDIFF(d,[postdatetime],getdate()) = 0 AND [uid] = @userid GROUP BY [extcredits]",
                                                BaseConfigs.GetTablePrefix);
            return DbHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        public int CreateUser(UserInfo userInfo)
        {
            DbParameter[] parms = {
                                       ...
                                       DbHelper.MakeInParam("@passkey",(DbType)SqlDbType.Char,32,userInfo.Passkey),
                                       DbHelper.MakeInParam("@inviterid",(DbType)SqlDbType.Int,4,userInfo.Inviterid),
									   ...
									   DbHelper.MakeInParam("@regip",(DbType)SqlDbType.VarChar,45,userInfo.Regip),
									   DbHelper.MakeInParam("@joindate",(DbType)SqlDbType.VarChar,0,userInfo.Joindate),
									   DbHelper.MakeInParam("@lastip",(DbType)SqlDbType.Char,45,userInfo.Lastip),
                                       ...
                                       //////////////////////////////////////////////////////////////////////////
                                       //【BT修改】
									   DbHelper.MakeInParam("@credits",(DbType)SqlDbType.Decimal,18,userInfo.Credits),
									   DbHelper.MakeInParam("@extcredits1",(DbType)SqlDbType.Decimal,8,userInfo.Extcredits1),
									   DbHelper.MakeInParam("@extcredits2",(DbType)SqlDbType.Decimal,8,userInfo.Extcredits2),
									   DbHelper.MakeInParam("@extcredits3",(DbType)SqlDbType.Decimal,32,userInfo.Extcredits3),
									   DbHelper.MakeInParam("@extcredits4",(DbType)SqlDbType.Decimal,32,userInfo.Extcredits4),
									   DbHelper.MakeInParam("@extcredits5",(DbType)SqlDbType.Decimal,18,userInfo.Extcredits5),
									   DbHelper.MakeInParam("@extcredits6",(DbType)SqlDbType.Decimal,18,userInfo.Extcredits6),
									   DbHelper.MakeInParam("@extcredits7",(DbType)SqlDbType.Decimal,32,userInfo.Extcredits7),
									   DbHelper.MakeInParam("@extcredits8",(DbType)SqlDbType.Decimal,32,userInfo.Extcredits8),
                                       //【END BT修改】
                                       //////////////////////////////////////////////////////////////////////////
                                       //////////////////////////////////////////////////////////////////////////
        ...
        /// <param name="modip"></param>
        /// <param name="section"></param>
        /// <param name="originalvalue"></param>
        /// <param name="newvalue"></param>
        public void InsertUserFieldHistory(int uid, int moduid, string modip, string section, string originalvalue, string newvalue)
        {
            DbParameter[] parms = {
									   DbHelper.MakeInParam("@uid", (DbType)SqlDbType.Int, 4, uid), 
									   DbHelper.MakeInParam("@moduid", (DbType)SqlDbType.Int, 4, moduid),
                                       DbHelper.MakeInParam("@modip",(DbType)SqlDbType.VarChar,45, modip),
                                       DbHelper.MakeInParam("@moddatetime",(DbType)SqlDbType.DateTime,8, DateTime.Now),
                                       DbHelper.MakeInParam("@section",(DbType)SqlDbType.VarChar,20, section),
                                       DbHelper.MakeInParam("@originalvalue", (DbType)SqlDbType.Text, 2000, originalvalue),
                                       DbHelper.MakeInParam("@newvalue", (DbType)SqlDbType.Text, 2000, newvalue),
								   };

            string commandText = "INSERT INTO [dnt_userfields_history] ([uid], [moddatetime], [moduid], [modip], [section], [originalvalue], [newvalue]) VALUES " +
                "(@uid, @moddatetime, @moduid, @modip, @section, @originalvalue, @newvalue)";
        ...
        public float GetUserExtCredits(int uid, int extId)
        {
            ...
            return (float)TypeConverter.ObjectToDecimal(DbHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0].Rows[0][0]);
        ...
        public void UpdateUserLastvisit(int uid, string ip)
        {
            DbParameter[] parms = {
									   DbHelper.MakeInParam("@uid", (DbType)SqlDbType.Int, 4, uid),
									   DbHelper.MakeInParam("@ip", (DbType)SqlDbType.Char,45, ip)
								   };
        ...
        public int AddOnlineUser(OnlineUserInfo onlineUserInfo, int timeOut, int deletingFrequency)
        {
            ...
            DbParameter[] parms = {
									   ...
									   DbHelper.MakeInParam("@ip",(DbType)SqlDbType.VarChar,45,onlineUserInfo.Ip),
                                       ...
                                       DbHelper.MakeInParam("@lastcheckintime",(DbType)SqlDbType.DateTime,8,onlineUserInfo.LastCheckInTime),
                                       DbHelper.MakeInParam("@lastcheckincount",(DbType)SqlDbType.Int,4,onlineUserInfo.LastCheckInCount),
                                       ...
                                       DbHelper.MakeInParam("@rkey",(DbType)SqlDbType.Char,10,onlineUserInfo.RKey),
                                       DbHelper.MakeInParam("@rkeyexpire",(DbType)SqlDbType.Char,10,onlineUserInfo.RkeyExpire),
                                       DbHelper.MakeInParam("@rkeyexpiretime",(DbType)SqlDbType.DateTime,8,onlineUserInfo.RkeyExpireTime),


                                       DbHelper.MakeInParam("@pmsound",(DbType)SqlDbType.TinyInt,1,onlineUserInfo.PMSound),
        ...
        private void DeleteExpiredOnlineUsers(int timeOut)
        {
            ...
            while (dr.Read())
            {
                //此处是否需要添加每次执行数量限制，比如1000个用户？？？ 更高效率的方式，直接转换为inttable，不经过字符串？？？
            ...
            {
                DataTable dt = PTTools.GetIntTableFromString(timeoutStrBuilder.ToString());
                ...
                DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, string.Format("{0}deleteonlineusers", BaseConfigs.GetTablePrefix), prams);
            ...
            {
                DataTable dt = PTTools.GetIntTableFromString(memberStrBuilder.ToString());
                DbParameter[] prams = new DbParameter[]{
                            new SqlParameter("@TVP", dt),
						};
                ((SqlParameter)prams[0]).SqlDbType = SqlDbType.Structured;
                ...
                DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, string.Format("{0}updateuseronlinestates", BaseConfigs.GetTablePrefix), prams);
        ...
        /// 更新用户在线时间
        /// </summary>
        /// <param name="oltimeSpan"></param>
        /// <param name="uid"></param>
        public void UpdateOnlineTime(int oltimeSpan, int uid)
        {
            DbParameter[] parms = {
                                    DbHelper.MakeInParam("@uid", (DbType)SqlDbType.Int, 4, uid),
                                    DbHelper.MakeInParam("@oltimespan", (DbType)SqlDbType.Int, 4, oltimeSpan),
        ...
        public void SynchronizeOnlineTime(int uid)
        {
            ...
            DbParameter[] parms2 = {
                                    DbHelper.MakeInParam("@uid", (DbType)SqlDbType.Int, 4, uid),
                                    DbHelper.MakeInParam("@oltime", (DbType)SqlDbType.Int, 4, total),
                                  };
            commandText = string.Format("UPDATE [{0}users] SET [oltime] = @oltime WHERE [oltime] < @oltime AND [uid] = @uid", BaseConfigs.GetTablePrefix, total);

            //如果更新失败，说明user表的oltime大于onlinetime表的total，则反向更新？？？ 为什么会出现这种异常情况
            if (DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms2) < 1)
            {
                try
                {
        ...
        public decimal GetUserExtCreditsByUserid(int uid, int extNumber)
        {
            string commandText = string.Format("SELECT TOP 1 [extcredits{0}] FROM [{1}users] WHERE [uid] = {2}",
                                                extNumber,
                                                BaseConfigs.GetTablePrefix,
                                                uid);
            return Convert.ToDecimal(TypeConverter.ObjectToFloat(DbHelper.ExecuteScalar(CommandType.Text, commandText), 0));
        }
