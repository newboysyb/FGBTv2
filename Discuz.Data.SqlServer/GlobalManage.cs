    ...
    public partial class DataProvider : IDataProvider
    {
        ...
        public int AddCreditsLog(int uid, int fromTo, int sendCredits, int receiveCredits, float send, float receive, string payDate, int operation)
        {
            DbParameter[] parms = {
									   ...
									   DbHelper.MakeInParam("@receive",(DbType)SqlDbType.Float,4,receive),    //使用Float会造成小数点后位数过长，导致显示不友好。所以在此使用Decimal
									   DbHelper.MakeInParam("@paydate",(DbType)SqlDbType.DateTime,8,DateTime.Parse(payDate)),
									   DbHelper.MakeInParam("@operation",(DbType)SqlDbType.Int,4,operation)
								   };
            //string commandText = string.Format("INSERT INTO [{0}creditslog] ([uid],[fromto],[sendcredits],[receivecredits],[send],[receive],[paydate],[operation],[uidext3],[fromtoext3]) VALUES(@uid,@fromto,@sendcredits,@receivecredits,@send,@receive,@paydate,@operation," +
            //    "(SELECT CASE WHEN NOT EXISTS (SELECT [extcredits3] FROM [{0}users] WHERE [{0}users].[uid] = @uid) THEN 0 ELSE (SELECT [extcredits3] FROM [{0}users] WHERE [{0}users].[uid] = @uid) END), " +
            //    "(SELECT CASE WHEN NOT EXISTS (SELECT [extcredits3] FROM [{0}users] WHERE [{0}users].[uid] = @fromto) THEN 0 ELSE (SELECT [extcredits3] FROM [{0}users] WHERE [{0}users].[uid] = @fromto) END))",
            //                                    BaseConfigs.GetTablePrefix);
            //return DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);

            return DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, "dnt_addcreditslog", parms);
        }
        ...
        public DataTable GetCreditsLogList(int pageSize, int currentPage, int uid)
        {
            ...
            if (currentPage == 1)
                commandText = string.Format("SELECT TOP (@pagesize) {1}, [ufrom].[username] AS [fromuser], [uto].[username] AS [touser] FROM [{0}creditslog] AS [cl], [{0}users] AS [ufrom], [{0}users] AS [uto] WHERE [cl].[uid]=[ufrom].[uid] AND [cl].[fromto]=[uto].[uid] AND ([cl].[uid]= @uid OR [cl].[fromto] = @uid)  ORDER BY [id] DESC",
                                             BaseConfigs.GetTablePrefix,
                                             DbFields.CREDITS_LOG_JOIN);
            else
                commandText = string.Format("SELECT TOP (@pagesize) {1}, [ufrom].[username] AS [fromuser], [uto].[username] AS [touser] FROM [{0}creditslog] AS [cl], [{0}users] AS [ufrom], [{0}users] AS [uto] WHERE [id] < (SELECT MIN([id])  FROM (SELECT TOP (@pagetop) [id] FROM [{0}creditslog] WHERE [{0}creditslog].[uid] = @uid OR [{0}creditslog].[fromto] = @uid ORDER BY [id] DESC) AS tblTmp ) AND [cl].[uid]=[ufrom].[uid] AND [cl].[fromto]=[uto].[uid] AND ([cl].[uid] = @uid OR [cl].[fromto] = @uid) ORDER BY [cl].[id] DESC",
                                             BaseConfigs.GetTablePrefix,
                                             DbFields.CREDITS_LOG_JOIN);
        ...
        public int GetCreditsLogRecordCount(int uid)
        {
            ...
            string commandText = string.Format("SELECT COUNT(1) FROM [{0}creditslog] WHERE [uid] = @uid OR [fromto]= @uid",
                                                BaseConfigs.GetTablePrefix);
        ...
        public void AddVisitLog(int uid, string userName, int groupId, string groupTitle, string ip, string actions, string others)
        {
            DbParameter[] parms = {
					...
					DbHelper.MakeInParam("@ip", (DbType)SqlDbType.VarChar, 45, ip),
        ...
        public void DeleteVisitLogs()
        {
            ...
            DbHelper.ExecuteNonQuery(CommandType.Text, string.Format("UPDATE [{0}adminvisitlog] SET [del] = 1", BaseConfigs.GetTablePrefix));
        }
        ...
        public void DeleteVisitLogs(string condition)
        {
            ...
            string commandText =  string.Format("UPDATE [{0}adminvisitlog] SET [del] = 1 WHERE [del] = 0 AND {1}", BaseConfigs.GetTablePrefix, condition);
        ...
        public DataTable GetVisitLogList(int pageSize, int currentPage)
        {
            ...
            //【BT修改】不删除后台记录
            if (currentPage == 1)
                commandText = string.Format("SELECT TOP {0} {1} FROM [{2}adminvisitlog] WHERE [del] = 0 ORDER BY [visitid] DESC",
                                             pageSize,
                                             DbFields.ADMIN_VISIT_LOG,
                                             BaseConfigs.GetTablePrefix);
            else
                commandText = string.Format("SELECT TOP {0} {1} FROM [{2}adminvisitlog]  WHERE [del] = 0 AND [visitid] < (SELECT MIN([visitid]) FROM (SELECT TOP {3} [visitid] FROM [{2}adminvisitlog] WHERE [del] = 0 ORDER BY [visitid] DESC) AS tblTmp )  ORDER BY [visitid] DESC",
                                             pageSize,
                                             DbFields.ADMIN_VISIT_LOG,
                                             BaseConfigs.GetTablePrefix,
                                             pagetop);
        ...
        {
            //【BT修改】不删除后台记录
            if (condition == "") return GetVisitLogList(pageSize, currentPage);

            int pagetop = (currentPage - 1) * pageSize;
            string commandText;

            if (currentPage == 1)
                commandText = string.Format("SELECT TOP {0} {1} FROM [{2}adminvisitlog] WHERE [del] = 0 AND {3} ORDER BY [visitid] DESC",
                                             pageSize,
                                             DbFields.ADMIN_VISIT_LOG,
                                             BaseConfigs.GetTablePrefix,
                                             condition);
            else
                commandText = string.Format("SELECT TOP {0} {1} FROM [{2}adminvisitlog]  WHERE [visitid] < (SELECT MIN([visitid])  FROM (SELECT TOP {3} [visitid] FROM [{2}adminvisitlog] WHERE [del] = 0 AND {4} ORDER BY [visitid] DESC) AS tblTmp ) AND [del] = 0 AND {4} ORDER BY [visitid] DESC",
                                             pageSize,
                                             DbFields.ADMIN_VISIT_LOG,
                                             BaseConfigs.GetTablePrefix,
                                             pagetop,
        ...
        {
            //【BT修改】不删除后台记录
            string commandText = string.Format("SELECT COUNT(visitid) FROM [{0}adminvisitlog] WHERE [del] = 0", BaseConfigs.GetTablePrefix);
        ...
        {
            //【BT修改】不删除后台记录
            if (condition == "") return GetVisitLogCount();
            string commandText = string.Format("SELECT COUNT(visitid) FROM [{0}adminvisitlog] WHERE [del] = 0 AND {1}", BaseConfigs.GetTablePrefix, condition);
            return TypeConverter.ObjectToInt(DbHelper.ExecuteDataset(CommandType.Text, commandText).Tables[0].Rows[0][0]);
        }
        ...
        public bool InsertModeratorLog(string moderatorUid, string moderatorName, string groupId, string groupTitle, string ip, string postDateTime, string fid, string fname, string tid, string title, string actions, string reason)
        {
            try
            {
                ...
                DbHelper.ExecuteNonQuery(CommandType.StoredProcedure, "dnt_insertmoderatorlog", parms);
        ...
        public int AddErrLoginRecord(string ip)
        {
            ...
            string commandText = string.Format("DELETE FROM [{0}failedlogins] WHERE [ip] = @ip AND [errcount] < 2;INSERT INTO [{0}failedlogins] ([ip], [errcount], [lastupdate]) VALUES(@ip, 1, GETDATE())",
                                                BaseConfigs.GetTablePrefix);
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public int ResetErrLoginCount(string ip)
        {
            ...
            //"UPDATE [{0}failedlogins] SET [errcount]=1, [lastupdate]=GETDATE() WHERE [ip]=@ip"
            //修改为，重置时，不更新时间，且要求时间必须大于15分钟，删除的时候只能删除15分钟之后的
            string commandText = string.Format("UPDATE [{0}failedlogins] SET [errcount] = 0 WHERE [ip] = @ip AND DATEDIFF(n,[lastupdate], GETDATE()) > 15",
                                                BaseConfigs.GetTablePrefix);
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public int DeleteErrLoginRecord(string ip)
        {
            ...
            string commandText = string.Format("DELETE FROM [{0}failedlogins] WHERE [errcount] = 0 AND [ip] = @ip AND DATEDIFF(n,[lastupdate], GETDATE()) > 15",
                                                BaseConfigs.GetTablePrefix);
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
        }
        ...
        public DataTable GetPayLogInList(int pageSize, int currentPage, int uid)
        {
            ...
            if (currentPage == 1)
                commandText = string.Format("SELECT TOP (@pagesize) {0}, [{1}topics].[fid] AS fid ,[{1}topics].[postdatetime] AS postdatetime ,[{1}topics].[poster] AS authorname, [{1}topics].[title] AS title,[{1}users].[username] AS UserName FROM [{1}paymentlog] AS [pl] LEFT OUTER JOIN [{1}topics] ON [pl].[tid] = [{1}topics].[tid] LEFT OUTER JOIN [{1}users] ON [{1}users].[uid] = [pl].[uid] WHERE [pl].[authorid]= @uid  ORDER BY [pl].[id] DESC",
                                             DbFields.PAYMENT_LOG_JOIN,
                                             BaseConfigs.GetTablePrefix);
            else
                commandText = string.Format("SELECT TOP (@pagesize) {1}, [{1}topics].[fid] AS fid ,[{1}topics].[postdatetime] AS postdatetime ,[{1}topics].[poster] AS authorname, [{1}topics].[title] AS title,[{1}users].[username] AS UserName FROM [{1}paymentlog] AS [pl] LEFT OUTER JOIN [{1}topics] ON [pl].[tid] = [{1}topics].[tid] LEFT OUTER JOIN [{1}users] ON [{1}users].[uid] = [pl].[uid] WHERE [id] < (SELECT MIN([id]) FROM (SELECT TOP (@pagetop) [id] FROM [{1}paymentlog] WHERE [{1}paymentlog].[authorid] = @uid ORDER BY [id] DESC) AS tblTmp ) AND [pl].[authorid] = @uid ORDER BY [pl].[id] DESC",
                                             DbFields.PAYMENT_LOG_JOIN,
                                             BaseConfigs.GetTablePrefix);
        ...
        public DataTable GetPayLogOutList(int pageSize, int currentPage, int uid)
        {
            ...
            if (currentPage == 1)
                commandText = string.Format("SELECT TOP (@pagesize) {0}, [{1}topics].[fid] AS fid ,[{1}topics].[postdatetime] AS postdatetime ,[{1}topics].[poster] AS authorname, [{1}topics].[title] AS title,[{1}users].[username] AS UserName FROM [{1}paymentlog] AS [pl] LEFT OUTER JOIN [{1}topics] ON [pl].[tid] = [{1}topics].[tid] LEFT OUTER JOIN [{1}users] ON [{1}users].[uid] = [pl].[uid] WHERE [pl].[uid] = @uid  ORDER BY [pl].[id] DESC",
                                             DbFields.PAYMENT_LOG_JOIN,
                                             BaseConfigs.GetTablePrefix);
            else
                commandText = string.Format("SELECT TOP (@pagesize) {0}, [{1}topics].[fid] AS fid ,[{1}topics].[postdatetime] AS postdatetime ,[{1}topics].[poster] AS authorname, [{1}topics].[title] AS title,[{1}users].[username] AS UserName FROM [{1}paymentlog] AS [pl] LEFT OUTER JOIN [{1}topics] ON [pl].[tid] = [{1}topics].[tid] LEFT OUTER JOIN [{1}users] ON [{1}users].[uid] = [pl].[uid] WHERE [id] < (SELECT MIN([id])  FROM (SELECT TOP (@pagetop) [id] FROM [{1}paymentlog] WHERE [{1}paymentlog].[uid] = @uid ORDER BY [id] DESC) AS tblTmp ) AND [pl].[uid] = @uid ORDER BY [pl].[id] DESC",
                                             DbFields.PAYMENT_LOG_JOIN,
                                             BaseConfigs.GetTablePrefix);
        ...
        public DataTable GetPaymentLogByTid(int pageSize, int currentPage, int tid)
        {
            ...
            if (currentPage == 1)
                commandText = string.Format("SELECT TOP (@pagesize) {0}, [{1}users].[username] AS username FROM [{1}paymentlog] AS [pl] LEFT OUTER JOIN [{1}topics] ON [pl].[tid] = [{1}topics].[tid] LEFT OUTER JOIN [{1}users] ON [{1}users].[uid] = [pl].[uid] WHERE [pl].[tid] = @tid  ORDER BY [pl].[id] DESC",
                                             DbFields.PAYMENT_LOG_JOIN,
                                             BaseConfigs.GetTablePrefix);
            else
                commandText = string.Format("SELECT TOP (@pagesize) {0}, [{1}users].[username] AS username FROM [{1}paymentlog] AS [pl] LEFT OUTER JOIN [{1}topics] ON [pl].[tid] = [{1}topics].[tid] LEFT OUTER JOIN [{1}users] ON [{1}users].[uid] = [pl].[uid] WHERE [id] < (SELECT MIN([id])  FROM (SELECT TOP (@pagetop) [id] FROM [{1}paymentlog] WHERE [{1}paymentlog].[tid]={4} ORDER BY [id] DESC) AS tblTmp ) AND [pl].[tid] = @tid ORDER BY [pl].[id] DESC",
                                             DbFields.PAYMENT_LOG_JOIN,
                                             BaseConfigs.GetTablePrefix);
        ...
        private string GetSearchTopicTitleSQL(int posterId, string searchForumId, int resultOrder, int resultOrderType, int searchTime, int searchTimeType, int digest, string keyWord, int postTableId)
        {
            ...
            // 替换转义字符 挪到主函数，因为使用了表值参数 //这个函数有bug。。。没赋值哎。。。。
            //strKeyWord.Replace("'", "''");
            //strKeyWord.Replace("%", "[%]");
            ...
            if (posterId > 0)
                sqlBuilder.AppendFormat(" AND [posterid] = @posterid ");

            if (digest > 0)
                sqlBuilder.Append(" AND [digest] > 0 ");

            if (searchTime != 0)
                sqlBuilder.AppendFormat(" AND [postdatetime] {2} @postdatetime",
                                          BaseConfigs.GetTablePrefix,
                                          postTableId,
                                          searchTimeType == 1 ? "<" : ">");
            ...
            if (keyWord.Length > 0)
            {
                ...
                {
                    //strKeyWord.Append(" AND [title], LIKE @keywords1 ");
                    strKeyWord.Append(" AND CONTAINS([title], @keywords1) ");
                }
                else if (keywordlist.Length > 1 && keywordlist.Length < 9)
                {
                    for(int i = 1; i < keywordlist.Length + 1; i++)
                    {
                        if (i == 1) strKeyWord.Append(" AND (CONTAINS([title], @keywords" + i.ToString() + ")");
                        else if(i == keywordlist.Length) strKeyWord.Append(" OR CONTAINS([title], @keywords" + i.ToString() + ")) ");
                        else strKeyWord.Append(" OR CONTAINS([title], @keywords" + i.ToString() + ")");
                    }
                }
                //超过8个关键词是否应该拒绝查询。。。。
                else
        ...
        private string GetSearchPostContentSQL(int posterId, string searchForumId, int resultOrder, int resultOrderType, int searchTime, int searchTimeType, int postTableId, StringBuilder strKeyWord)
        {
            ...
            if (keywordlist.Length > 0)
            {
                ...
                {
                    strKeyWord.Append(" AND CONTAINS(message, @keywords1) ");
                }
                else if (keywordlist.Length > 1 && keywordlist.Length < 9)
                {
                    for (int i = 1; i < keywordlist.Length + 1; i++)
                    {
                        if (i == 1) strKeyWord.Append(" AND (CONTAINS(message, @keywords" + i.ToString() + ")");
                        else if (i == keywordlist.Length) strKeyWord.Append(" OR CONTAINS(message, @keywords" + i.ToString() + ")" + ") ");
                        else strKeyWord.Append(" OR CONTAINS(message, @keywords" + i.ToString() + ")");
                    }
                }
                //超过8个关键词是否应该拒绝查询。。。。
                else
        ...
        /// 按照用户搜索帖子？？？
        /// </summary>
        /// <param name="spaceEnabled"></param>
        /// <param name="albumEnabled"></param>
        ...
        private string GetSearchByPosterSQL(bool spaceEnabled, bool albumEnabled, int posterId, int postTableId)
        {
            if (posterId > 0)
            {
                ...
                sqlBuilder.AppendFormat(@"SELECT DISTINCT [tid], 'forum' AS [datafrom] FROM [{0}posts{1}] WHERE [posterid] = @posterid AND [tid] NOT IN (SELECT [tid] FROM [{0}topics] WHERE [posterid] = @posterid AND [displayorder] < 0)",
                                          BaseConfigs.GetTablePrefix,
                                          postTableId,
                                          posterId);
        ...
        public int Search(bool spaceEnabled, bool albumEnabled, int postTableId, int userId, int userGroupId, string keyWord, int posterId, SearchType searchType, string searchForumId, int searchTime, int searchTimeType, int resultOrder, int resultOrderType)
        {
            ...
            // 超过60分钟的缓存纪录将被删除
            // 此处可用户改进，增加热门搜索关键词功能
            // 保留72小时查询记录，增加查询次数项目，增加updatecache功能，选择热门查询词
            ...
            //////////////////////////////////////////////////////////////////////////
            //【BT修改】forumid 表值化，LIKE参数化
            // 此函数是否需要移动到业务层，不保留在数据层？？？

            //groupid实际并未使用，因此在保存cache的时候，只需要保存实际搜索的forumid即可，不需要保存groupid了

            string forumids = searchForumId;
            DataTable dt = PTTools.GetIntTableFromString(searchForumId);
            if (dt.Rows.Count > 0) searchForumId = "SELECT [IntValue] FROM @TVP";

            string[] keywordlist = Utils.SplitString(keyWord.ToString(), " ");
            //超过8个关键词不查询
            if (keywordlist.Length > 8) return -2;
            ...
            //查找缓存表，是否存在缓存
            DbParameter[] prams2 = {
										DbHelper.MakeInParam("@searchstring",(DbType)SqlDbType.VarChar,400, sql),
										DbHelper.MakeInParam("@uid",(DbType)SqlDbType.Int,4,userId),
										DbHelper.MakeInParam("@groupid",(DbType)SqlDbType.Int,4,userGroupId),
                                        DbHelper.MakeInParam("@keywords",(DbType)SqlDbType.NVarChar, 100, keyWord),
                                        DbHelper.MakeInParam("@forumids",(DbType)SqlDbType.VarChar, 200, forumids),
                                        DbHelper.MakeInParam("@posterid",(DbType)SqlDbType.Int,4,posterId),
                                        DbHelper.MakeInParam("@postdatetime",(DbType)SqlDbType.DateTime, 8, DateTime.Now.AddDays(searchTime)),
									};
            int searchid = Utils.StrToInt(
                DbHelper.ExecuteScalar(CommandType.Text, string.Format(
                    @"SELECT TOP 1 [searchid] FROM [{0}searchcaches] WHERE [searchstring] = @searchstring AND [keywords] = @keywords AND [forumids] = @forumids AND posterid = @posterid",
                    BaseConfigs.GetTablePrefix), prams2), -1);
            if (searchid > -1 && searchTime == 0) //按照时间查询的查询结果没有办法缓存。。。时间时刻都在变化中，而且无法精确化
                return searchid;

            //没有缓存，进行查询
            DbParameter[] prams = new DbParameter[11];
            if(searchType == SearchType.PostContent || searchType == SearchType.TopicTitle || searchType == SearchType.DigestTopic)
            {
                prams = new DbParameter[]{
                            new SqlParameter("@TVP", dt),
                            DbHelper.MakeInParam("@keywords1",(DbType)SqlDbType.NVarChar, 50, keywordlist.Length > 0 ? "\"*" + keywordlist[0] + "*\"" : ""),
                            DbHelper.MakeInParam("@keywords2",(DbType)SqlDbType.NVarChar, 50, keywordlist.Length > 1 ? "\"*" + keywordlist[1] + "*\"" : ""),
                            DbHelper.MakeInParam("@keywords3",(DbType)SqlDbType.NVarChar, 50, keywordlist.Length > 2 ? "\"*" + keywordlist[2] + "*\"" : ""),
                            DbHelper.MakeInParam("@keywords4",(DbType)SqlDbType.NVarChar, 50, keywordlist.Length > 3 ? "\"*" + keywordlist[3] + "*\"" : ""),
                            DbHelper.MakeInParam("@keywords5",(DbType)SqlDbType.NVarChar, 50, keywordlist.Length > 4 ? "\"*" + keywordlist[4] + "*\"" : ""),
                            DbHelper.MakeInParam("@keywords6",(DbType)SqlDbType.NVarChar, 50, keywordlist.Length > 5 ? "\"*" + keywordlist[5] + "*\"" : ""),
                            DbHelper.MakeInParam("@keywords7",(DbType)SqlDbType.NVarChar, 50, keywordlist.Length > 6 ? "\"*" + keywordlist[6] + "*\"" : ""),
                            DbHelper.MakeInParam("@keywords8",(DbType)SqlDbType.NVarChar, 50, keywordlist.Length > 7 ? "\"*" + keywordlist[7] + "*\"" : ""),
                            DbHelper.MakeInParam("@posterid",(DbType)SqlDbType.Int,4,posterId),
                            DbHelper.MakeInParam("@postdatetime",(DbType)SqlDbType.DateTime, 8, DateTime.Now.AddDays(searchTime)),
						};
                ((SqlParameter)prams[0]).SqlDbType = SqlDbType.Structured;
                ((SqlParameter)prams[0]).TypeName = "fgbtIntTable";
            }
            else
            {
                prams = new DbParameter[]{
            ...
            if (reader != null)
            {
                switch (searchType)
                {
                    ...
                    case SearchType.ByPoster:
                        ...
                        cacheinfo.Forumids = forumids;
                        cacheinfo.PosterId = posterId;
                ...
                //if (rowcount > 0)
                {
                    if (strTids.ToString() != "<ForumTopics>" && strTids.ToString() != "<SpacePosts>" && strTids.ToString() != "<Albums>")
        ...
        {
            //////////////////////////////////////////////////////////////////////////
            //【BT修改】与Interop.SQLDMO.dll有关的SQL操作，屏蔽
        ...
        public void UpdateStatCount(string browser, string os, string visitorsAdd, bool sessionexist, bool isguest)
        {
            string month = DateTime.Now.Year + DateTime.Now.Month.ToString("00");
            string dayofweek = ((int)DateTime.Now.DayOfWeek).ToString();

            //commandText = string.Format("UPDATE [{0}stats] SET [count]=[count]+1 WHERE ([type]='total' AND [variable]='hits') {1} OR ([type]='month' AND [variable]='{2}') OR ([type]='week' AND [variable]='{3}') OR ([type]='hour' AND [variable]='{4}')",
                                                //BaseConfigs.GetTablePrefix,
                                                //visitorsAdd,
                                                //month,
                                                //dayofweek,
            ...
            //如果更新条数小于应该的数量，则按照单条目更新，创建不存在的条目（月份更新之类）
            bool domoreupdate = false;

            //更新数据
            if (sessionexist)
            {
                DbParameter[] parms = { 
                                        DbHelper.MakeInParam("@month", (DbType)SqlDbType.Char, 20, month),
                                        DbHelper.MakeInParam("@dayofweek", (DbType)SqlDbType.Char, 20, dayofweek),
                                        DbHelper.MakeInParam("@hourofday", (DbType)SqlDbType.Char, 20, DateTime.Now.Hour.ToString("00")),
                                  };
                string commandText = string.Format(
                    "UPDATE [{0}stats] SET [count] = [count] + 1 WHERE ([type] = 'total' AND [variable] = 'hits') OR ([type]='month' AND [variable]= @month)" +
                    "OR ([type]='week' AND [variable]= @dayofweek) OR ([type]='hour' AND [variable] = @hourofday)", BaseConfigs.GetTablePrefix);

                int affectedrows = DbHelper.ExecuteNonQuery(CommandType.Text, commandText);
                if (affectedrows < 4) domoreupdate = true;
            }
            if (sessionexist)
            {
                DbParameter[] parms = { 
                                        ...
                                        DbHelper.MakeInParam("@dayofweek", (DbType)SqlDbType.Char, 20, dayofweek),
                                        DbHelper.MakeInParam("@hourofday", (DbType)SqlDbType.Char, 20, DateTime.Now.Hour.ToString("00")),
                                        DbHelper.MakeInParam("@browser", (DbType)SqlDbType.Char, 20, browser),
                                        DbHelper.MakeInParam("@os", (DbType)SqlDbType.Char, 20, os),
                                        DbHelper.MakeInParam("@guestsormembers", (DbType)SqlDbType.Char, 20, isguest ? "guests" : "members"),
                                  };
                string commandText = string.Format(
                    "UPDATE [{0}stats] SET [count] = [count] + 1 WHERE ([type] = 'total' AND [variable] = 'hits') OR ([type]='month' AND [variable]= @month)" +
                    "OR ([type]='browser' AND [variable]= @browser) OR ([type]='os' AND [variable] = @os) OR ([type]='total' AND [variable] = @guestsormembers)" +
                    "OR ([type]='week' AND [variable]= @dayofweek) OR ([type]='hour' AND [variable] = @hourofday)", BaseConfigs.GetTablePrefix);

                int affectedrows = DbHelper.ExecuteNonQuery(CommandType.Text, commandText);
                if (affectedrows < 7) domoreupdate = true;
            }
        ...
        public IDataReader GetNoticeByUid(int uid, NoticeType noticeType)
        {
            ...
            return DbHelper.ExecuteReader(CommandType.StoredProcedure, string.Format("{0}getnoticebyuid", BaseConfigs.GetTablePrefix), parms);
        }
        ...
        public IDataReader GetNoticeByUid(int uid, NoticeType noticeType, int pageId, int pageSize)
        {
            ...
            if (pageId == 1)
            {
                commandText = string.Format("SELECT TOP (@pagesize) [nid], [uid], [type], [new], [posterid], [poster], [note], [postdatetime]  FROM [{0}notices] WHERE [uid] = @uid{1} ORDER BY [postdatetime] DESC",
                                            BaseConfigs.GetTablePrefix,
                                            noticeType == NoticeType.All ? "" : " AND [type]= @type");
            }
            else
            {
                commandText = string.Format("SELECT TOP (@pagesize) [nid], [uid], [type], [new], [posterid], [poster], [note], [postdatetime]  FROM [{0}notices] WHERE [nid] < (SELECT MIN([nid])  FROM (SELECT TOP (@pagetop) [nid] FROM [{0}notices] WHERE [uid] = @uid{1} ORDER BY [postdatetime] DESC) AS tblTmp ) AND [uid] = @uid{1} ORDER BY [postdatetime] DESC",
                                            BaseConfigs.GetTablePrefix,
                                            noticeType == NoticeType.All ? "" : " AND [type]= @type");
            }
        ...
        //                          };
        //    string commandText = "SELECT TOP 1 nid, uid, type, new, posterid, poster, note, postdatetime FROM [dnt_notices] WHERE [nid] = @nid";
        //    return DbHelper.ExecuteReader(CommandType.Text, commandText, parms);
        //}
