...
namespace Discuz.Data.SqlServer
{
    public partial class DataProvider : IDataProvider
    {
        ...
        public DataTable GetFocusTopicList(int count, int views, int fid, string typeIdList, string startTime, string orderFieldName, string visibleForum, bool isDigest, bool onlyImg)
        {
            ...
            string fidParam = (fid <= 0 ? "" : string.Format(" AND ([t].[fid] = @fid OR CHARINDEX(',' + @fid + ',' , ',' + RTRIM([f].[parentidlist]) + ',') > 0 ) ", fid));
            ...
            if (visibleForum == "4") fid = 4;
            else if (visibleForum == "34") fid = 34;
            else if (visibleForum == "37") fid = 37;

            DataTable dtvForum = PTTools.GetIntTableFromString(visibleForum);
            DataTable dttList = PTTools.GetIntTableFromString(typeIdList);

            if (!Utils.StrIsNullOrEmpty(visibleForum))
                visibleForum = string.Format(" AND [t].[fid] IN (SELECT [IntValue] FROM @vForum)");

            string typeidParam = "";
            if (!string.IsNullOrEmpty(typeIdList) && Utils.IsNumericList(typeIdList))
            {
                typeidParam = string.Format(" AND [t].[typeid] IN (SELECT [IntValue] FROM @tList)");
            }
            ...
            //不显示关闭的主题版本
            //string sqlstr = string.Format(@"SELECT TOP {0} {1}, [f].[name], [ff].[rewritename] FROM [{2}topics] AS [t] LEFT JOIN [{2}forums] AS [f] ON [t].[fid] = [f].[fid] LEFT JOIN [{2}forumfields] ff ON [f].[fid] = [ff].[fid] WHERE [t].[displayorder] >=0 AND [t].[views] > {3} AND [t].[postdatetime] > @starttime{4}{5} AND [ff].[password] ='' ORDER BY [t].[{6}] DESC",
            //                       count,
            ...
            DbParameter[] parms = {
                                      new SqlParameter("@vForum", dtvForum),
                                      new SqlParameter("@tList", dttList),
									  DbHelper.MakeInParam("@views", (DbType)SqlDbType.Int,4, views),
                                      DbHelper.MakeInParam("@pagesize", (DbType)SqlDbType.Int,4, count),
                                      ...
                                      DbHelper.MakeInParam("@endtime", (DbType)SqlDbType.DateTime, 8, DateTime.Now.AddHours(-18)),
			                      };
            ((SqlParameter)parms[0]).SqlDbType = SqlDbType.Structured;
            ((SqlParameter)parms[0]).TypeName = "fgbtIntTable";
            ((SqlParameter)parms[1]).SqlDbType = SqlDbType.Structured;
            ((SqlParameter)parms[1]).TypeName = "fgbtIntTable";

            //增加标签版本，fid需要在上面赋值，否则不起作用
            if (fid == 4)
            {
                //公告区显示关闭的主题
                sqlstr = string.Format(@"SELECT TOP (@pagesize) {0}, [f].[name], [ff].[rewritename], [tt].[name] AS [typename] FROM [{1}topics] AS [t] LEFT JOIN [{1}forums] AS [f] ON [t].[fid] = [f].[fid] LEFT JOIN [{1}forumfields] AS [ff] ON [f].[fid] = [ff].[fid] LEFT JOIN [{1}topictypes] AS [tt] ON [t].[typeid] = [tt].[typeid] WHERE [t].[displayorder] >=0 AND [t].[views] > @views AND [t].[postdatetime] > @starttime{2}{3} AND [ff].[password] ='' ORDER BY [t].[{4}] DESC",
                           DbFields.TOPICS_JOIN,
                           BaseConfigs.GetTablePrefix,
                           fidParam + digestParam + visibleForum + typeidParam,
                           attParam,
            ...
            else if(fid == 34 || fid == 37)
            {
                //新手版、跳蚤版 只显示12小时之前发布的主题
                sqlstr = string.Format(@"SELECT TOP (@pagesize) {0}, [f].[name], [ff].[rewritename], [tt].[name] AS [typename] FROM [{1}topics] AS [t] LEFT JOIN [{1}forums] AS [f] ON [t].[fid] = [f].[fid] LEFT JOIN [{1}forumfields] AS [ff] ON [f].[fid] = [ff].[fid] LEFT JOIN [{1}topictypes] AS [tt] ON [t].[typeid] = [tt].[typeid] WHERE [t].[closed] = 0 AND [t].[displayorder] >=0 AND [t].[views] > @views AND [t].[postdatetime] < @endtime AND [t].[postdatetime] > @starttime{2}{3} AND [ff].[password] ='' ORDER BY [t].[{4}] DESC",
                           DbFields.TOPICS_JOIN,
                           BaseConfigs.GetTablePrefix,
                           fidParam + digestParam + visibleForum + typeidParam,
                           attParam,
            ...
            {
                sqlstr = string.Format(@"SELECT TOP (@pagesize) {0}, [f].[name], [ff].[rewritename], [tt].[name] AS [typename] FROM [{1}topics] AS [t] LEFT JOIN [{1}forums] AS [f] ON [t].[fid] = [f].[fid] LEFT JOIN [{1}forumfields] AS [ff] ON [f].[fid] = [ff].[fid] LEFT JOIN [{1}topictypes] AS [tt] ON [t].[typeid] = [tt].[typeid] WHERE [t].[closed] = 0 AND [t].[displayorder] >=0 AND [t].[views] > @views AND [t].[postdatetime] > @starttime{2}{3} AND [ff].[password] ='' ORDER BY [t].[{4}] DESC",
                           DbFields.TOPICS_JOIN,
                           BaseConfigs.GetTablePrefix,
                           fidParam + digestParam + visibleForum + typeidParam,
                           attParam,
        ...
        public int GetTodayPostCount(int fid, string postTableName)
        {
            ...
            string commandText = string.Format("SELECT COUNT([pid]) AS [postcount] FROM [{0}] WHERE [fid] = @fid AND DATEDIFF(day, [postdatetime], GETDATE()) = 0",
                                                postTableName);
            return TypeConverter.ObjectToInt(DbHelper.ExecuteScalar(CommandType.Text, commandText, parms));
        }
        ...
        {
            //原存储过程 string.Format("{0}getdesignatepostcount"
            DbParameter[] parms = {
									   DbHelper.MakeInParam("@fid",(DbType)SqlDbType.Int,4,fid),
									   //DbHelper.MakeInParam("@posttablename", (DbType)SqlDbType.VarChar, 50, postTableName)
								   };
            string commandText = string.Format("SELECT COUNT([pid]) AS [postcount] FROM [{0}] WHERE [fid] = @fid", postTableName);
            return TypeConverter.ObjectToInt(DbHelper.ExecuteScalar(CommandType.Text, commandText, parms));
        }
        ...
        public IDataReader GetPollOptionList(int tid)
        {
            ...
            string commandText = string.Format("SELECT {0} FROM [{1}polloptions] WHERE [tid]=@tid Order By [displayorder] ASC, [polloptionid] ASC",
        ...
        public string GetPollEnddatetime(int tid)
        {
            ...
            string commandText = string.Format("SELECT TOP 1 [expiration] FROM [{0}polls] WHERE [tid] = @tid",
                                                BaseConfigs.GetTablePrefix,
                                                tid);
            return Utils.GetDate(DbHelper.ExecuteScalarToStr(CommandType.Text, commandText, parms), Utils.GetDate());
        }
        ...
        public int CreatePost(PostInfo postInfo, string postTableId)
        {
            DbParameter[] parms = {
                                       ...
                                       DbHelper.MakeInParam("@rootparentid",(DbType)SqlDbType.Int,4,postInfo.RootParentid),
									   DbHelper.MakeInParam("@parentid",(DbType)SqlDbType.Int,4,postInfo.Parentid),
                                       DbHelper.MakeInParam("@replied",(DbType)SqlDbType.TinyInt,1,postInfo.Replied),
									   DbHelper.MakeInParam("@layer",(DbType)SqlDbType.Int,4,postInfo.Layer),
									   DbHelper.MakeInParam("@poster",(DbType)SqlDbType.VarChar,20,postInfo.Poster), //修改为20长度
                                       ...
                                       DbHelper.MakeInParam("@title",(DbType)SqlDbType.NVarChar,255,postInfo.Title),
                                       DbHelper.MakeInParam("@topictitle",(DbType)SqlDbType.NVarChar,255,postInfo.Topictitle),
                                       DbHelper.MakeInParam("@ip",(DbType)SqlDbType.VarChar,45,postInfo.Ip),
									   ...
									   DbHelper.MakeInParam("@invisible",(DbType)SqlDbType.SmallInt,2,postInfo.Invisible),
									   DbHelper.MakeInParam("@usesig",(DbType)SqlDbType.TinyInt,1,postInfo.Usesig),
									   DbHelper.MakeInParam("@htmlon",(DbType)SqlDbType.TinyInt,1,postInfo.Htmlon),
									   DbHelper.MakeInParam("@smileyoff",(DbType)SqlDbType.TinyInt,1,postInfo.Smileyoff),
									   DbHelper.MakeInParam("@bbcodeoff",(DbType)SqlDbType.TinyInt,1,postInfo.Bbcodeoff),
									   DbHelper.MakeInParam("@parseurloff",(DbType)SqlDbType.TinyInt,1,postInfo.Parseurloff),
        ...
        /// <param name="postTableName"></param>
        /// <returns></returns>
        public int UpdatePostSetRepliedTrue(int pid, string postTableName)
        {
            DbParameter[] parms = {
									   DbHelper.MakeInParam("@pid",(DbType)SqlDbType.Int,4,pid),
                                  };
            string commandText = string.Format("UPDATE [{0}] SET [replied] = 1 WHERE [pid] = @pid", postTableName);
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms); 
        }
        ...
        /// <param name="postTableName"></param>
        /// <returns></returns>
        public int UpdatePostSetReplied(int pid, int tid, string postTableName)
        {
            DbParameter[] parms = {
									   DbHelper.MakeInParam("@pid",(DbType)SqlDbType.Int,4,pid),
                                       DbHelper.MakeInParam("@tid",(DbType)SqlDbType.Int,4,tid),
                                  };
            string commandText = string.Format("UPDATE [{0}] SET [replied] = (CASE WHEN EXISTS(SELECT [pid] FROM [{0}] WHERE [tid] = @tid AND [parentid] = @pid AND [invisible] <= 0) THEN 1 ELSE 0 END) WHERE [pid] = @pid", postTableName);
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms); 
        }
        ...
        public int UpdatePost(PostInfo postsInfo, string postTableId, int uid, string ip)
        {
            ...
            if (uid > 0)
            {
                DbParameter[] parms = {
                                       ...
                                       DbHelper.MakeInParam("@title",(DbType)SqlDbType.NVarChar,255,postsInfo.Title),
									   ...
									   DbHelper.MakeInParam("@invisible",(DbType)SqlDbType.SmallInt,2,postsInfo.Invisible),
									   DbHelper.MakeInParam("@usesig",(DbType)SqlDbType.TinyInt,1,postsInfo.Usesig),
									   DbHelper.MakeInParam("@htmlon",(DbType)SqlDbType.TinyInt,1,postsInfo.Htmlon),
									   DbHelper.MakeInParam("@smileyoff",(DbType)SqlDbType.TinyInt,1,postsInfo.Smileyoff),
									   DbHelper.MakeInParam("@bbcodeoff",(DbType)SqlDbType.TinyInt,1,postsInfo.Bbcodeoff),
									   DbHelper.MakeInParam("@parseurloff",(DbType)SqlDbType.TinyInt,1,postsInfo.Parseurloff),


                                       DbHelper.MakeInParam("@modip",(DbType)SqlDbType.VarChar, 45,ip),
                                       DbHelper.MakeInParam("@moduid",(DbType)SqlDbType.Int,4,uid),
                                       DbHelper.MakeInParam("@modtype",(DbType)SqlDbType.Int,4, 0),
                                       DbHelper.MakeInParam("@moddatetime",(DbType)SqlDbType.DateTime,8,DateTime.Now),
								   };
                return DbHelper.ExecuteNonQuery(CommandType.StoredProcedure,
                                            string.Format("{0}updatepost{1}_withhistory", BaseConfigs.GetTablePrefix, postTableId),
                                            parms);
            }
            else
            {
        ...
        public IDataReader GetPostInfo(string postTableId, int pid)
        {
            ...
            string commandText = string.Format("SELECT TOP 1 {0} FROM [{1}posts{2}] WHERE [pid] = @pid",
                                                DbFields.POSTS,
                                                BaseConfigs.GetTablePrefix,
                                                postTableId);
            return DbHelper.ExecuteReader(CommandType.Text, commandText, parms);
        ...
        public DataTable GetPostList(string topicList, string[] postTableId)
        {
            ...
            //多个分表同时读取的情况【是否有此必要？？？】此处需要想办法进行替代以进行参数化
            ...
            {
                //单个分表读取
                DataTable dt = PTTools.GetIntTableFromString(topicList);
                DbParameter[] parms = {
                ...
                string posttable = postTableId.Length == 1 ? postTableId[0] : "1";
                string commandText = string.Format("SELECT {0} FROM [{1}posts{2}] WHERE [tid] IN (SELECT [IntValue] FROM @TVP)", DbFields.POSTS, BaseConfigs.GetTablePrefix, posttable);
                return DbHelper.ExecuteDataset(CommandType.Text, commandText, parms).Tables[0];
            }
       }
        ...
        public int UpdatePostRateTimes(string postIdList, string postTableId)
        {
            ...
            {
                int pidint = TypeConverter.StrToInt(pid, -1);
                if (pidint < 1) continue;

                DbParameter[] parms = {
									  DbHelper.MakeInParam("@pid", (DbType)SqlDbType.Int,4, pidint)
			                      };
                commandText = string.Format("UPDATE [{0}posts{1}] SET [ratetimes] = (SELECT COUNT([uid]) FROM [{0}ratelog] WHERE [{0}ratelog].[pid] = @pid) WHERE [{0}posts{1}].[pid] = @pid",
                    BaseConfigs.GetTablePrefix, postTableId);

                rst += DbHelper.ExecuteNonQuery(CommandType.Text, commandText, parms);
            }
            return rst;
        }
        ...
        {
            DbParameter[] parms = {
									   DbHelper.MakeInParam("@keywords",(DbType)SqlDbType.NVarChar,100,cacheInfo.Keywords),
									   DbHelper.MakeInParam("@searchstring",(DbType)SqlDbType.VarChar,400,cacheInfo.Searchstring),
                                       DbHelper.MakeInParam("@forumids",(DbType)SqlDbType.VarChar,200,cacheInfo.Forumids),
                                       DbHelper.MakeInParam("@posterid",(DbType)SqlDbType.Int,4,cacheInfo.PosterId),
									   DbHelper.MakeInParam("@ip",(DbType)SqlDbType.VarChar,60,cacheInfo.Ip),
        ...
        public void DeleteExpriedSearchCache()
        {
            DbHelper.ExecuteNonQuery(CommandType.Text,
                                     ...
                                     DbHelper.MakeInParam("@expiration", (DbType)SqlDbType.DateTime, 8, DateTime.Now.AddMinutes(-60).ToString("yyyy-MM-dd HH:mm:ss")));
        }
        ...
        public DataTable GetSearchCache(int searchId)
        {
            return DbHelper.ExecuteDataset(CommandType.Text,
                                           string.Format("SELECT TOP 1 [tids],[keywords],[forumids],[posterid] FROM [{0}searchcaches] WHERE [searchid]=@searchid", BaseConfigs.GetTablePrefix),
        ...
        public DataTable GetSearchTopicsList(int pageSize, string strTids)
        {
            ...
            DataTable dt = PTTools.GetIntTableFromString(strTids);
            ...
            //string commandText = string.Format("SELECT TOP (@pagesize) [{0}topics].[tid], [{0}topics].[title], [{0}topics].[poster], [{0}topics].[posterid], [{0}topics].[postdatetime], [{0}topics].[replies], [{0}topics].[views], [{0}topics].[lastpost],[{0}topics].[lastposter],[{0}topics].[lastposterid], [{0}forums].[fid],[{0}forums].[name] AS [forumname] FROM [{0}topics] LEFT JOIN ([{0}forums] LEFT JOIN [{0}forumfields] ON [{0}forums].[fid]=[{0}forumfields].[fid]) ON [{0}forums].[fid] = [{0}topics].[fid] WHERE [{0}topics].[tid] IN (SELECT [IntValue] FROM @TVP) AND [{0}forumfields].[password]='' AND CHARINDEX(CONVERT(VARCHAR(8),[{0}topics].[tid]),'{2}') > 0 ORDER BY CHARINDEX(LTRIM([{0}topics].[tid]),'{2}')",
            //            BaseConfigs.GetTablePrefix);
            //string commandText = string.Format("SELECT TOP (@pagesize) [{0}topics].[tid], [{0}topics].[title], [{0}topics].[poster], [{0}topics].[posterid], [{0}topics].[postdatetime], [{0}topics].[replies], [{0}topics].[views], [{0}topics].[lastpost],[{0}topics].[lastposter],[{0}topics].[lastposterid], [{0}forums].[fid],[{0}forums].[name] AS [forumname] FROM [{0}topics] LEFT JOIN ([{0}forums] LEFT JOIN [{0}forumfields] ON [{0}forums].[fid]=[{0}forumfields].[fid]) ON [{0}forums].[fid] = [{0}topics].[fid] WHERE [{0}topics].[tid] IN (SELECT [IntValue] FROM @TVP) AND [{0}forumfields].[password]='' ORDER BY (SELECT ROW_NUMBER() OVER(ORDER BY (SELECT 0)) AS ROWNUM FROM @TVP WHERE [IntValue] = [{0}topics].[tid])",
            //                    BaseConfigs.GetTablePrefix);       
            //【BT修改】表值化 较为复杂。。。暂时不动，需要按照tid顺序排序
            //DataTable dt = PTTools.GetIntTableFromString(strTids);

            string commandText = string.Format("SELECT TOP (@pagesize) [{0}topics].[tid], [{0}topics].[title], [{0}topics].[poster], [{0}topics].[posterid], [{0}topics].[postdatetime], [{0}topics].[replies], [{0}topics].[views], [{0}topics].[lastpost],[{0}topics].[lastposter],[{0}topics].[lastposterid], [{0}forums].[fid],[{0}forums].[name] AS [forumname] FROM [{0}topics] LEFT JOIN ([{0}forums] LEFT JOIN [{0}forumfields] ON [{0}forums].[fid]=[{0}forumfields].[fid]) ON [{0}forums].[fid] = [{0}topics].[fid] WHERE [{0}topics].[tid] IN (SELECT [IntValue] FROM @TVP) AND [{0}forumfields].[password]='' ORDER BY [{0}topics].[tid] DESC",
                                    BaseConfigs.GetTablePrefix);
        ...
        public int SetTopicStatus(string topicList, string field, string intValue)
        {
            ...
            string commandText = string.Format("UPDATE [{0}topics] SET [{1}] = @field WHERE [tid] IN (SELECT [IntValue] FROM @TVP)",
                                                BaseConfigs.GetTablePrefix,
                                                field,
                                                topicList);
            return DbHelper.ExecuteNonQuery(CommandType.Text, commandText,parms);
        ...
        public int SetTopicClose(string topicList, short intValue)
        {
            ...
            string commandText = string.Format("UPDATE [{0}topics] SET [closed] = @field WHERE [tid] IN (SELECT [IntValue] FROM @TVP) AND [closed] IN (0,1)",
                                                BaseConfigs.GetTablePrefix);
        ...
        public int GetTopicStatus(string topicList, string field)
        {
            ...
            string commandText = string.Format("SELECT SUM(ISNULL([{0}],0)) AS [fieldcount] FROM [{1}topics] WHERE [tid] IN (SELECT [IntValue] FROM @TVP)",
                                                field,
                                                BaseConfigs.GetTablePrefix);
            return TypeConverter.ObjectToInt(DbHelper.ExecuteDataset(CommandType.Text, commandText, parms).Tables[0].Rows[0][0]);
        }

        
        //从数据库删除关闭的主题，无用，取消函数20140429
        ...
        public IDataReader GetTopicListModeratorLog(int tid)
        {
            ...
            string commandText = string.Format("SELECT TOP 1 [grouptitle], [moderatorname],[postdatetime],[actions] FROM [{0}moderatormanagelog] WHERE [tid] = @tid ORDER BY [id] DESC",
                                                BaseConfigs.GetTablePrefix);
            return DbHelper.ExecuteReader(CommandType.Text, commandText, parms);
        }
        ...
        public int GetTopicReplyCountByUserId(int userId)
        {
            ...
            string commandText = string.Format("SELECT COUNT(DISTINCT [tid]) FROM [{0}myposts] WHERE [uid] = @userid",
                                                BaseConfigs.GetTablePrefix);
            return TypeConverter.ObjectToInt(DbHelper.ExecuteScalar(CommandType.Text, commandText, parms));
        }
        ...
        {
            DataTable dt = PTTools.GetIntTableFromString(attachIdList);
            //if (dt.Rows.Count < 1) return new IDataReader(); 空表格是否会造成异常情况？？？
            DbParameter[] parms = {
