...
namespace Discuz.Forum
{
    ...
    public class Posts
    {
        ...
        /// <returns></returns>
        public static int CheckReplyToPoster(int replypostid, string message, string title)
        {
            Regex r1 = new Regex(@"\[b\]回复\s+\[url=showtopic.aspx\?topicid=\d+&postid=(\d+)#\d+]\d+楼");
            if (r1.IsMatch(message))
            {
                if (r1.Match(message).Groups[1].Value == replypostid.ToString()) return 1;
                else return -1;
            }
            else
            {
                Regex r2 = new Regex(@"回复\s+\d+楼.+的帖子#(\d+)#");
                if (r2.IsMatch(title))
                {
                    if (r2.Match(title).Groups[1].Value == replypostid.ToString()) return 2;
                    else return -2;
                }
                else return -3;
            } 
        ...
        /// <returns></returns>
        public static int UpdatePostSetRepliedTrue(int pid, int tid)
        {
            return DatabaseProvider.GetInstance().UpdatePostSetRepliedTrue(pid, PostTables.GetPostTableName(tid));
        }        
        /// <summary>
        /// 设置帖子是否被回复
        /// </summary>
        ...
        /// <param name="postTableName"></param>
        /// <returns></returns>
        public static int UpdatePostSetReplied(int pid, int tid)
        {
            return DatabaseProvider.GetInstance().UpdatePostSetReplied(pid, tid, PostTables.GetPostTableName(tid));
        }
        ...
        public static int CreatePost(PostInfo postInfo)
        {
            ...
            //////////////////////////////////////////////////////////////////////////
            ////////////////////////////////////////////////////////////////////////// 
            //【BT修改】at用户功能create 和update

            PrivateBT.GetPostContentAtUser(pid, postInfo.Tid);
        ...
        /// <param name="postsInfo">帖子信息</param>
        /// <returns>更新数量</returns>
        public static int UpdatePost(PostInfo postInfo, int uid, string ip)
        {
            ...
            //////////////////////////////////////////////////////////////////////////
            ////////////////////////////////////////////////////////////////////////// 
            //【BT修改】at用户功能,先保存帖子内容，再更新AT用户功能
            
            int rtvalue =  Data.Posts.UpdatePost(postInfo, GetPostTableId(postInfo.Tid), uid, ip);

            PrivateBT.GetPostContentAtUser(postInfo.Pid, postInfo.Tid);
            return rtvalue;
        ...
        public static DataTable GetPostTree(int tid, int hide, decimal userCredit)
        {
        ...
        private static void LoadPostMessage(PostpramsInfo postpramsInfo, List<ShowtopicPageAttachmentInfo> attachList, bool isModer, int allowGetAttach, int originalHideStatus, ShowtopicPagePostInfo postInfo)
        {
            ...
            if (!showMessage && isModer)
            {
                ...
                postInfo.Signature = "<div class='hintinfo'>该用户签名档内容已被屏蔽, 您拥有管理权限, 以下是签名档内容</div>" + postInfo.Signature;
            }
            //这是个种子帖子，总是显示被屏蔽的主题帖
            else if (!showMessage && postpramsInfo.Topicinfo.SeedId > 0 && postInfo.Layer == 0 && postInfo.Id == 1)
            {
                showMessage = true;
            }
            else if (!showMessage)
        ...
        /// <param name="_postpramsinfo">参数列表</param>
        /// <returns>帖子的信息</returns>
        public static ShowtopicPagePostInfo GetSinglePostForRSS(PostpramsInfo postPramsInfo, out Discuz.Common.Generic.List<ShowtopicPageAttachmentInfo> attachmentList, bool ismoder)
        {
            //此代码已被移至showdebate.aspx.cs页面
            //UserInfo userInfo = Users.GetUserInfo(postPramsInfo.CurrentUserid);
            //postPramsInfo.Usercredits = userInfo == null ? 0 : userInfo.Credits;
            ...
            if (postInfo != null)
            {
                ...
                if (postInfo.Attachment > 0 || regexAttach.IsMatch(message) || regexAttachImg.IsMatch(message))
                {
                    ...
                    string[] attHidArray = GetHiddenAttachIdList(postPramsInfo.Sdetail, postPramsInfo.Hide);
                    List<ShowtopicPageAttachmentInfo> attachDeleteList = new List<ShowtopicPageAttachmentInfo>();
                    foreach (ShowtopicPageAttachmentInfo attach in attachmentList)
                    {
                        message = Attachments.GetMessageWithAttachInfo(postPramsInfo, allowGetAttach, attHidArray, postInfo, attach, message);
                        //if (attach.Inserted == 1 && (postpramsInfo.CurrentUserGroup.Radminid == 1 || attach.Uid == postpramsInfo.CurrentUserid || attach.Attachprice <= 0 || attach.Isbought == 1)//当为发帖人或不为收费附件或已购买该收费附件时
                        //    || Utils.InArray(attach.Aid.ToString(), attHidArray))
        ...
        /// <param name="topiclistnodepath"></param>
        /// <param name="websitetopiclistnodepath"></param>
