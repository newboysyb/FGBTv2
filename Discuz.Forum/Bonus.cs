...
namespace Discuz.Forum
{
    ...
    public class Bonus
    {
        ...
        public static void CloseBonus(TopicInfo topicinfo, int userid, int[] postIdArray, int[] winerIdArray, string[] winnerNameArray, string[] costBonusArray, string[] valuableAnswerArray, int bestAnswer)
        {
            ...
            if (System.DateTime.Parse(topicinfo.Postdatetime) > System.DateTime.Parse("2012-05-28 15:00"))
            {
                Users.UpdateUserExtCredits(topicinfo.Posterid, Scoresets.GetBonusCreditsTrans(), topicinfo.Price * 1024f * 1024 * 1024);
                CreditsLogs.AddCreditsLog(topicinfo.Posterid, topicinfo.Posterid, 3, 3, 0, topicinfo.Price * 1024f * 1024 * 1024, Utils.GetDateTime(), 12);
            }
            ...
            for (int i = 0; i < winerIdArray.Length; i++)
            {
                ...
                if (winerIdArray[i] > 0 && bonus > 0)
                {
                    ...
                    Users.UpdateUserExtCredits(winerIdArray[i], Scoresets.GetBonusCreditsTrans(), bonus * 1024f * 1024 * 1024);
                    CreditsLogs.AddCreditsLog(topicinfo.Posterid, winerIdArray[i], 3, 3, 0, bonus * 1024f * 1024 * 1024, Utils.GetDateTime(), 12);
