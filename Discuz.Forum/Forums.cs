...
namespace Discuz.Forum
{
    ...
    public class Forums
    {
        ...
        /// <param name="usergroupid">用户组id</param>
        /// <returns></returns>
        public static bool AllowView(ForumInfo forum, int usergroupid, string ipaddress)
        {
            //根据ip地址不显示的特定板块
            if ((forum.Allowpostspecial & 64) == 64 && !PTAbt.IsIPAllowed(ipaddress)) return false;

            return HasPerm(forum.Viewperm, usergroupid);
        }
        ...
        /// </summary>
        /// <returns>板块信息数组</returns>
        public static List<ForumHottopicsList> GetForumHotTopicsList()
        {
            DNTCache cache = DNTCache.GetCacheService();
            List<ForumHottopicsList> forumhottopicsList = cache.RetrieveObject("/Forum/ForumHotTopicsList") as List<ForumHottopicsList>;

            if (forumhottopicsList == null || forumhottopicsList.Count == 0)
            {
                forumhottopicsList = Discuz.Data.Forums.GetForumHotTopicsList();
                cache.AddObject("/Forum/ForumHotTopicsList", forumhottopicsList, 600);
            }
            return forumhottopicsList;
        }

        public static ForumHottopicsList GetForumHotTopics(int fid)
        {
            if (fid < 1)
                return new ForumHottopicsList();

            List<ForumHottopicsList> forumhottopicsList = GetForumHotTopicsList();
            if (forumhottopicsList == null)
                return new ForumHottopicsList();

            foreach (ForumHottopicsList forumhottopics in forumhottopicsList)
            {
                if (forumhottopics.Fid == fid)
                {
                    return forumhottopics;
                }
            }
            return new ForumHottopicsList();
        }
        ...
        /// <summary>
        /// 返回全部版块列表并缓存，过滤资源发布板块的显示
        /// </summary>
        /// <returns>板块信息数组</returns>
        public static List<ForumInfo> GetForumListProcessBTForum()
        {
            List<ForumInfo> forumList = new List<ForumInfo>(GetForumList().ToArray().Clone() as ForumInfo[]);
            List<ForumInfo> forumlist2 = new List<ForumInfo>();

            //种子列表区不显示版块描述
            foreach (ForumInfo f in forumList)
            {    
                forumlist2.Add(f.Clone());
            }
            foreach (ForumInfo f in forumlist2)
            {
                if (PrivateBT.Forum2Type(f.Fid) > 0)
                {
                    f.Description = "资源发布板块";
                }
            }
            return forumlist2;
        }
        ...
        public static string GetVisibleForum(string ipaddress)
        {
            StringBuilder result = new StringBuilder();
            List<ForumInfo> forumList = GetVisibleForumList(ipaddress);
        ...
        public static List<ForumInfo> GetVisibleForumList(string ipaddress)
        {
            List<ForumInfo> forumList = GetForumList();
            ...
            foreach (ForumInfo foruminfo in forumList)
            {
                if (foruminfo.Status > 0)
                {
                    ...
                    {
                        if (!AllowView(foruminfo, 7, ipaddress))
                            continue;
                    }
                    //当版块未设置密码时
        ...
        //////////////////////////////////////////////////////////////////////////
        ////////////////////////////////////////////////////////////////////////// 
        //【BT修改】按照groupid获取可见板块列表

        /// <summary>
        /// 获得可见的板块列表
        /// </summary>
        /// <returns>返回值是以英文逗号分割的板块ID</returns>
        public static string GetVisibleForum(decimal ext2, int groupid, string ipaddress)
        {
            StringBuilder result = new StringBuilder();
            List<ForumInfo> forumList = GetVisibleForumList(ext2, groupid, ipaddress);
        ...
        public static List<ForumInfo> GetVisibleForumList(decimal ext2, int groupid, string ipaddress)
        {
            List<ForumInfo> forumList = GetForumList();
            List<ForumInfo> visibleForumList = new List<ForumInfo>();
            ...
            foreach (ForumInfo foruminfo in forumList)
            {
                if (foruminfo.Status > 0)
                {
                    ...
                    if (Utils.StrIsNullOrEmpty(foruminfo.Viewperm)) //当板块权限为空时，按照用户组权限
                    {
                        if (!AllowView(foruminfo, groupid, ipaddress))
                            continue;
                        if (UserGroups.GetUserGroupInfo(groupid).Allowvisit != 1)
                            continue;
                        //当金币小于10个时，不显示分享版
                        if (foruminfo.Fid == 58 && ext2 < 10) continue;
                    }
                    else //当板块权限不为空，按照板块权限
                    {
                        if (!AllowView(foruminfo, groupid, ipaddress))
        ...
        private static void UpdateParentForumLastPost(ForumInfo foruminfo, PostInfo postinfo)
        {
            ...
            if (parentFid > 0)
            {
                ...
                //获取到游客可以访问到的版块fidlist//这里获取的fidlist是 传递进来的子版块，对应父板块的全部可见子版块，而不是只有上面的子版块，
        ...
        /// <returns></returns>
        public static Discuz.Common.Generic.List<IndexPageForumInfo> GetForumIndexCollection(int hidePrivate, int userGroupId, int moderStyle, out int topicCount, out int postCount, out int todayCount, string ipaddress)
        {
            ...
            {
                //根据ip地址不显示的特定板块
                if ((info.Allowpostspecial & 64) == 64 && !PTAbt.IsIPAllowed(ipaddress)) continue;

                //if (info.Allowspecialonly == 64)
                //{
                //    if (!PTAbt.IsIPAllowed(ipaddress))
                //        continue;
                //}

                //if ((info.Allowpostspecial & 64) == 64)
                //{
                //    if (!PTAbt.IsIPAllowed(ipaddress))
                //        continue;
                //}
        ...
        /// <returns></returns>
        public static Discuz.Common.Generic.List<IndexPageForumInfo> GetSubForumCollection(int fid, int colcount, int hideprivate, int usergroupid, int moderstyle, string ipaddress)
        {
            Discuz.Common.Generic.List<IndexPageForumInfo> coll = new Discuz.Common.Generic.List<IndexPageForumInfo>();
        ...
        /// <summary>
        /// 由特殊帖子描述获取特殊帖子id，即字段special、digest值
        /// </summary>
        /// <param name="special"></param>
        /// <returns></returns>
        public static int GetTopicSpecialID(string special)
        {
            switch (special.ToLower().Trim())//如果此处要加入新分支，则showforum.aspx.cs  中  SetSearchCondition()  方法中也需要添加一个，具体的看代码就明白了
            {
                case "poll":
                    return 1;
                case "reward":
                    return -2; //special值2或3
                case "rewarded":
                    return 3;
                case "rewarding":
                    return 2;
