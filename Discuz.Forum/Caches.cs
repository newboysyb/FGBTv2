...
namespace Discuz.Forum
{
    ...
    public class Caches
    {
        ...
        /// <param name="cateUnselectable">是否不可选择版块分类</param>
        /// <returns>列表内容的html</returns>
        public static string GetForumListBoxOptionsCache(int groupid)
        {
            string str = GetForumListBoxOptionsCache(true);

            if (groupid != 1)
            {
                str = str.Replace("<option value=\"53\"> &nbsp;&nbsp;真正的管理员站务区^_^</option>", "");
                if (groupid != 3)
                {
                    str = str.Replace("<optgroup label=\"--管理员站务区\">管理员站务区<option value=\"39\"> &nbsp;&nbsp;管理员站务</option></optgroup>", "");
                }
            }
            return str;
        }
