    ...
    public class Advertisements
    {
        ...
        public static string GetSelectStr(string pagename, int fid, AdType adtype)
        {
            ...
            if (!Utils.StrIsNullOrEmpty(pagename) && pagename == "indexad")
                selectstr = ",首页,";
            if (!Utils.StrIsNullOrEmpty(pagename) && pagename == "loginad")
                selectstr = ",登陆页面,";
            if (!Utils.StrIsNullOrEmpty(pagename) && pagename == "registerad")
                selectstr = ",注册页面,";
            else
            {
                if (fid > 0)
        ...
        public static string GetInPostAd(string pagename, int forumid, string templatepath, int count)
        {
            ...
            if (adshowArray.Length > 0)
            {
                sb.Append("<div style=\"display: none;\" id=\"ptnotice_none\">\r\n");
                //帖内下方的广告
        ...
        public static string GetAdShowInfo(AdShowInfo[] adshowArray, int count, int inPostAdType)
        {
            ...
            for (int i = 1; i <= count; i++)
            {
                ...
                if (tmp.Count > 0)
                {
                    ...
                    {
                        case 0: //帖内下方的广告
                            adsMsg += string.Format("<div class=\"ptnotice_textlink1\" id=\"ptnotice_thread1_{0}_none\">{1}</div>\r\n", i, tmp[random.Next(0, tmp.Count)].Code); break;
                        case 1: //帖内上方的广告
                            adsMsg += string.Format("<div class=\"ptnotice_textlink2\" id=\"ptnotice_thread2_{0}_none\">{1}</div>\r\n", i, tmp[random.Next(0, tmp.Count)].Code); break;
                        default: //帖内右方的广告
                            adsMsg += string.Format("<div class=\"ptnotice_pip\" id=\"ptnotice_thread3_{0}_none\">{1}</div>\r\n", i, tmp[random.Next(0, tmp.Count)].Code); break;
                    }
                }
            }
            return adsMsg;
        ...
        public static string GetInForumAd(string pagename, int forumid, List<IndexPageForumInfo> topforum, string templatepath)
        {
            ...
            if (adshowArray.Length > 0)
            {
                ...
                result.Append("<div style=\"display: none\" id=\"ptnotice_none\">\r\n");
                for (int i = 0; i < topforum.Count; i++)
                {
                    if (i >= adshowArray.Length)
                        break;

                    result.AppendFormat("<div class=\"ptnotice_column\" id=\"ptnotice_intercat_{0}_none\">{1}</div>\r\n", topforum[i].Fid, adshowArray[i].Code);
                }
        ...
        public static string GetAdShowInfoXMLByFloor(AdShowInfo[] adshowArray, int floor, int inPostAdType)
        {
            ...
            if (tmp.Count > 0)
            {
                ...
                adsMsg = string.Format("<ptnotice_thread{0}><![CDATA[{1}]]></ptnotice_thread{0}>", inPostAdType, ad.Code);
            }
            return adsMsg;
        }
