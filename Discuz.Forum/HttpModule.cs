    ...
    public class HttpModule : System.Web.IHttpModule
    {
        ...
        public void Init(HttpApplication context)
        {
            ...
            if (eventTimer == null && ScheduleConfigs.GetConfig().Enabled)
            {
                ...
                eventTimer = new Timer(new TimerCallback(ScheduledEventWorkCallback), context.Context, 30000, EventManager.TimerMinutesInterval * 30000);
            }
        ...
        private void ScheduledEventWorkCallback(object sender)
        {
            try
            {
                ...
                {
                    EventManager.Execute("Web");
                }
            }
            catch (System.Exception ex)
            {
                PTLog.InsertSystemLog(PTLog.LogType.ScheduledEventWeb, PTLog.LogStatus.Exception, "ScheduledEventW", "EX:" + ex.ToString());   
        ...
        private void ReUrl_BeginRequest(object sender, EventArgs e)
        {
            ...
            //使用版块名路径，即是用类似于http://buaabt.cn/seed/ 的路径作为某个版块路径的访问方式，实际只请求showforum.aspx页面
                ...
                requestPath.IndexOf("/tools/") < 0 && requestPath.IndexOf("/space/") < 0)
            {
                ...
                //【BT修改】手机版访问，强制样式更改位置1 of 2
                string agent = HttpContext.Current.Request.UserAgent;
                string hostname = HttpContext.Current.Request.Url.Host;
                if (strTemplateid != "5" && (System.Web.HttpContext.Current.Request.UserHostAddress == "xxx.xxx.xxx.xxx" || System.Web.HttpContext.Current.Request.UserHostAddress == "2001:da8:203:502:a8a1:1cad:5de5:47d1" || context.Request.Url.Host == "m.buaabt.cn" || context.Request.Url.Host == "mbuaa.buaabt.cn" || context.Request.Url.Host == "m1.buaabt.cn" || context.Request.Url.Host == "m2.buaabt.cn"))
                {
                    //IMAX过滤，不修改
                    if (System.Web.HttpContext.Current.Request.UserHostAddress == "xxx.xxx.xxx.xxx" && 
                        (System.Web.HttpContext.Current.Request.Url.Host == "ipv4.buaabt.cn" || System.Web.HttpContext.Current.Request.Url.Host == "buaabt.cn"
                    ...
                    else if (!agent.Contains("Windows NT") && !agent.Contains("Macintosh"))
                    {
                        strTemplateid = "5";
                    }
                }
                else if (strTemplateid == "5")
                {
                    if (System.Web.HttpContext.Current.Request.UserHostAddress != "xxx.xxx.xxx.xxx" && System.Web.HttpContext.Current.Request.UserHostAddress != "2001:da8:203:502:a8a1:1cad:5de5:47d1" && hostname != "ipv4.buaabt.cn" && hostname != "m.buaabt.cn" && hostname != "mbuaa.buaabt.cn" && hostname != "m1.buaabt.cn" && hostname != "m2.buaabt.cn")
                    {
                        strTemplateid = "1";
                    }
                ...
                if (path.Length > 1 && !Utils.StrIsNullOrEmpty(path[1]))
                {
                    ...
                    if (forumid > 0)
                    {
                        ...
                        //实际请求的页面地址showforum.aspx页面
                        if (strTemplateid == "5") //手机版
                            context.RewritePath(forumPath + "aspx/" + strTemplateid + "/showforum.aspx", string.Empty, newUrl + "&selectedtemplateid=" + strTemplateid);
                        else //其他方式，都是用模板1的页面
                            context.RewritePath(forumPath + "aspx/1/showforum.aspx", string.Empty, newUrl + "&selectedtemplateid=" + strTemplateid);
                ...
                if (requestPath.Substring(forumPath.Length).IndexOf("/") == -1)
                {
                    ...
                    if (requestPath.EndsWith("/index.aspx"))
                    {
                        if (config.Indexpage == 0)
                        {
                            ...
                            if (strTemplateid == "5") context.RewritePath(forumPath + "aspx/" + strTemplateid + "/forumindex.aspx");
                            else context.RewritePath(forumPath + "aspx/1/forumindex.aspx");
                        }
                        else
                        {
                            //if (config.BrowseCreateTemplate == 1)
                            ...
                            //}
                            if (strTemplateid == "5") context.RewritePath(forumPath + "aspx/" + strTemplateid + "/website.aspx");
                            else context.RewritePath(forumPath + "aspx/1/website.aspx");
                    ...
                    if (config.Aspxrewrite == 1)
                    {
                        foreach (SiteUrls.URLRewrite url in SiteUrls.GetSiteUrls().Urls)
                        {
                            if (Regex.IsMatch(requestPath, url.Pattern, Utils.GetRegexCompiledOptions() | RegexOptions.IgnoreCase))
                            {
                                ...
                                if (strTemplateid == "5") context.RewritePath(forumPath + "aspx/" + strTemplateid + url.Page, string.Empty, newUrl + "&selectedtemplateid=" + strTemplateid);
                                else context.RewritePath(forumPath + "aspx/1" + url.Page, string.Empty, newUrl + "&selectedtemplateid=" + strTemplateid);
                    ...
                    if (strTemplateid == "5")  context.RewritePath(forumPath + "aspx/" + strTemplateid + requestPath.Substring(context.Request.Path.LastIndexOf("/")), string.Empty, context.Request.QueryString.ToString() + "&selectedtemplateid=" + strTemplateid);
                    else context.RewritePath(forumPath + "aspx/1" + requestPath.Substring(context.Request.Path.LastIndexOf("/")), string.Empty, context.Request.QueryString.ToString() + "&selectedtemplateid=" + strTemplateid);
