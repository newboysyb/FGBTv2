...
namespace Discuz.Forum
{
    ...
    public class UBB
    {
        ...
        /// <summary>
        /// UBB代码处理函数【BT修改】配置为不显示图片时，将图片替换为链接
        /// </summary>
        ...
        public static string UBBToHTML(PostpramsInfo _postpramsinfo)
        {
            ...
            //表情处理，【此处包括连接内部内容】，但此处的链接均为由系统生成路径，无用户输入内容。
            
            #region 表情处理
            
            //清除无效的smilie标签（用户不应该直接输入此类型标签，所有输入均会被处理）
            ...
            if (_postpramsinfo.Bbcodeoff == 0)
            {
                ...
                // Bold, Italic, Underline 字体：粗体、斜体、下划线
                ...
                // Email ，【此处包括连接内部内容】，【此处包括用户输入内容】****需要进行XSS防范处理*** 
                // 所有用户输入的特殊字符已经经过转码，无需考虑
                // Email地址过滤调整：标签内不允许再出现[]:;"'换行等字符  原正则为@"\[email(?:\s*)\](.*?)\[\/email\]"
                // Email地址增强过滤：按照email规则，长度限制，@前64字符，@后255字符
                sDetail = Regex.Replace(sDetail, @"\[email(?:\s*)\]([^\[\]\n\t\v:;""']{1,64}@[^\[\]\n\t\v:;""']{1,255})\[\/email\]", "<a href=\"mailto:$1\" target=\"_blank\">$1</a>", options);
                // 增强过滤，原正则为@"\[email=(.[^\[]*)(?:\s*)\](.*?)\[\/email(?:\s*)\]"
                sDetail = Regex.Replace(sDetail, @"\[email=([^\[\]\n\t\v:;""']{1,64}@[^\[\]\n\t\v:;""']{1,255})(?:\s*)\](.*?)\[\/email(?:\s*)\]", "<a href=\"mailto:$1\" target=\"_blank\">$2</a>", options);

                // URL，【此处包括连接内部内容】，【此处包括用户输入内容】****需要进行XSS防范处理*** 
                ...
                int quotecount = 0;
                while (intQuoteIndexOf >= 0 && sDetail.ToLower().IndexOf("[/quote]") >= 0 && quotecount < 15) //【BT修改】 最多显示15层嵌套引用
                {
                    quotecount++;
            ...
            else//【BT修改】配置为不显示图片时，将图片替换为链接
                sDetail = ParseImgToLink(sDetail);
            ...
            //添加本站链接自动转换
            sDetail = ReplaceURL(sDetail);

            //<img>标签中，使用attachment.aspx?attachmentid=xxx表示的图片，全部转换为绝对地址
        ...
        /// <returns></returns>
        public static string ReplaceURL(string sDetail)
        {

            string replaceunit = @"bt.buaa6.edu.cn|buaabt.cn|ipv4.buaabt.cn|ipv6.buaabt.cn|bt.buaa.edu.cn|202.112.137.55|\[xxx.xxx.xxx.xxx\]";
            replaceunit += @"|ftp2.buaa.edu.cn|tracker4.buaabt.cn|tracker6.buaabt.cn";
            replaceunit += @"|\[2001:0da8:0203:0001:0000:0000:0000:0009\]|\[2001:da8:203:502:ed88:ab90:b967:a13c\]|\[2001:da8:203:502:35db:5634:4037:cbe8\]";

            Regex r1 = new Regex(@"(<a href=|<img src=)\""(http|https):\/\/(" + replaceunit + @")\/", RegexOptions.IgnoreCase);
            sDetail = r1.Replace(sDetail, "$1\"");

            //2001:da8:203:502:ed88:ab90:b967:a13c
            //sDetail = sDetail.Replace("<a href=\"http://bt.buaa6.edu.cn/", "<a href=\"");
            //sDetail = sDetail.Replace("<a href=\"http://buaabt.cn/", "<a href=\"");
            //sDetail = sDetail.Replace("<a href=\"http://[xxx.xxx.xxx.xxx]/", "<a href=\"");
            //sDetail = sDetail.Replace("<a href=\"http://202.112.137.55/", "<a href=\"");
            //sDetail = sDetail.Replace("<a href=\"http://[2001:0da8:0203:0001:0000:0000:0000:0009]/", "<a href=\"");


            //sDetail = sDetail.Replace("<img src=\"http://bt.buaa6.edu.cn/", "<img src=\"");
            //sDetail = sDetail.Replace("<img src=\"http://buaabt.cn/", "<img src=\"");
            //sDetail = sDetail.Replace("<img src=\"http://[xxx.xxx.xxx.xxx]/", "<img src=\"");
            //sDetail = sDetail.Replace("<img src=\"http://202.112.137.55/", "<img src=\"");
            //sDetail = sDetail.Replace("<img src=\"http://[2001:0da8:0203:0001:0000:0000:0000:0009]/", "<img src=\"");
        ...
        private static string ParseImg(string sDetail, int Signature)
        {
            ...
            {
                //限制img标签长度？
                sDetail = Regex.Replace(sDetail, @"\[img\]\s*([^\[\]\<\r\n\s]{1,2048}?)\s*\[\/img\]", IMG_SIGN_SIGNATURE, options);
                //sDetail = Regex.Replace(sDetail, @"\[img\]\s*([^\[\]\<\r\n\s]+?)\s*\[\/img\]", IMG_SIGN_SIGNATURE, options);
                //sDetail = Regex.Replace(sDetail, @"\[img\]\s*(http://[^\[\<\r\n]+?)\s*\[\/img\]", IMG_SIGN_SIGNATURE, options);
            }
            else
            {
                sDetail = Regex.Replace(sDetail, @"\[img\]\s*([^\[\]\<\r\n\s]{1,2048}?)\s*\[\/img\]", IMG_SIGN, options);
                //sDetail = Regex.Replace(sDetail, @"\[img\]\s*([^\[\]\<\r\n\s]+?)\s*\[\/img\]", IMG_SIGN, options);
                //sDetail = Regex.Replace(sDetail, @"\[img\]\s*(http://[^\[\<\r\n]+?)\s*\[\/img\]", IMG_SIGN, options);
            }

            sDetail = Regex.Replace(sDetail, @"\[img=(\d{1,4})[x|\,](\d{1,4})\]\s*([^\[\]\<\r\n\s]{1,2048}?)\s*\[\/img\]", "<img src=\"$3\" width=\"$1\" height=\"$2\" border=\"0\" onload=\"thumbImg(this)\" />", options).Replace("width=\"0\"", "").Replace("height=\"0\"", "");
            //sDetail = Regex.Replace(sDetail, @"\[img=(\d{1,4})[x|\,](\d{1,4})\]\s*([^\[\]\<\r\n\s]+?)\s*\[\/img\]", "<img src=\"$3\" width=\"$1\" height=\"$2\" border=\"0\" onload=\"thumbImg(this)\" />", options).Replace("width=\"0\"", "").Replace("height=\"0\"", "");
            ...
            sDetail = Regex.Replace(sDetail, @"\[image\]([\s\S]{1,2048}?)\[/image\]", "<img src=\"$1\" border=\"0\" />", options);
        ...
        ////////////////////////////////////////////////////////////////////////// 
        /// <summary>
        /// 【BT修改】添加函数 处理img标记，转换为链接，为手机版使用
        /// </summary>
        /// <param name="sDetail"></param>
        /// <returns></returns>
        private static string ParseImgToLink(string sDetail)
        {

            sDetail = Regex.Replace(sDetail, @"\[img\]\s*([^\[\<\r\n\s]{1,2048}?)\s*\[\/img\]", "<a href=\"$1\">[点击显示图片]</a>", options);

                //sDetail = Regex.Replace(sDetail, @"\[img\]\s*(http://[^\[\<\r\n]+?)\s*\[\/img\]", IMG_SIGN, options);

            sDetail = Regex.Replace(sDetail, @"\[img=(\d{1,4})[x|\,](\d{1,4})\]\s*([^\[\<\r\n\s]{1,2048}?)\s*\[\/img\]", "<a href=\"$3\">[点击显示图片]</a>", options);

            //sDetail = Regex.Replace(sDetail, @"\[img=(\d{1,4})[x|\,](\d{1,4})\]\s*(http://[^\[\<\r\n]+?)\s*\[\/img\]", "<img src=\"$3\" width=\"$1\" height=\"$2\" border=\"0\" onload=\"thumbImg(this)\" />", options);
            sDetail = Regex.Replace(sDetail, @"\[image\]([\s\S]{1,2048}?)\[/image\]", "<a href=\"$1\">[点击显示图片]</a>", options);
        ...
        {
            sDetail = Regex.Replace(sDetail, @"\[color=([^\[\<]+?)\]", "<span style=\"color:$1\">", options);
            ...
            sDetail = Regex.Replace(sDetail, @"\[float=(left|right)\]", "<span style=\"float: $1;\">", options); //<br style=\"clear: both\"><span style=\"float: $1;\">
            sDetail = Regex.Replace(sDetail, @"\[/color(?:\s*)\]", "</span>", options);
        ...
        public static string ParseUrl(string sDetail)
        {
            ...
            // URL中可以包括:和;
            // URL最大长度2048
            //不限制协议的方式是否有安全隐患？花园相对链接的方式 [url=showtopic.aspx]不包含冒号单独处理？其他包含冒号的网址 http://a.com:8080/ddd.html

            //普通URL，带www
            sDetail = Regex.Replace(sDetail, @"\[url(?:\s*)\](www\.([^\[\]\n\t\v""']{1,2048}?))\[/url(?:\s*)\]", "<a href=\"http://$1\" target=\"_blank\">$1</a>", options);
            sDetail = Regex.Replace(sDetail, @"\[url=www.([^\[\]\n\t\v""']{1,2048}?)(?:\s*)\]([\s\S]+?)\[/url(?:\s*)\]", "<a href=\"http://www.$1\" target=\"_blank\">$2</a>", options);

            //不带www的url，用于花园连接
            sDetail = Regex.Replace(sDetail, @"\[url(?:\s*)\]\s*(([^\[\]\n\t\v""':]{1,2048}?))\s*\[\/url(?:\s*)\]", "<a href=\"$1\" target=\"_blank\">$1</a>", options);
            sDetail = Regex.Replace(sDetail, @"\[url=(([^\[\]\n\t\v""':]{1,2048}?))(?:\s*)\]([\s\S]+?)\[/url(?:\s*)\]", "<a href=\"$1\" target=\"_blank\">$3</a>", options);
            
            
            //普通URL，带协议名称
            //【原版】sDetail = Regex.Replace(sDetail, @"\[url(?:\s*)\]\s*((mailto:|https?://|ftp://|gopher://|news://|telnet://|rtsp://|mms://|callto://|bctp://|ed2k://|tencent)([^\[""']+?))\s*\[\/url(?:\s*)\]", "<a href=\"$1\" target=\"_blank\">$1</a>", options);
            sDetail = Regex.Replace(sDetail, @"\[url(?:\s*)\]\s*((mailto:|https?://|ftp://|gopher://|news://|telnet://|rtsp://|mms://|callto://|bctp://|ed2k://|tencent)([^\[\]\n\t\v""']{1,2048}?))\s*\[\/url(?:\s*)\]", "<a href=\"$1\" target=\"_blank\">$1</a>", options);
            //不限制协议的方式是否有安全隐患？([^\[\]\n\t\v""']{1,2048}?)
            //【原版】sDetail = Regex.Replace(sDetail, @"\[url=((mailto:|https?://|ftp://|gopher://|news://|telnet://|rtsp://|mms://|callto://|bctp://|ed2k://|tencent://)([^\[""']+?))(?:\s*)\]([\s\S]+?)\[/url(?:\s*)\]", "<a href=\"$1\" target=\"_blank\">$4</a>", options);
            sDetail = Regex.Replace(sDetail, @"\[url=((mailto:|https?://|ftp://|gopher://|news://|telnet://|rtsp://|mms://|callto://|bctp://|ed2k://|tencent://)([^\[\]\n\t\v""']{1,2048}?))(?:\s*)\]([\s\S]+?)\[/url(?:\s*)\]", "<a href=\"$1\" target=\"_blank\">$4</a>", options);
        ...
        {
            sDetail = Regex.Replace(sDetail, @"((\r\n)*\[alert1\])(.*?)((\r\n)*\[\/alert1\])", "<span style=\"border:#C00 2px solid;color:#C00; font-size:16px; background-color:#FF0\"><b>$3</b></span>", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            sDetail = Regex.Replace(sDetail, @"((\r\n)*\[alert2\])(.*?)((\r\n)*\[\/alert2\])", "<span style=\"border:#00C 2px solid;color:#00C; font-size:16px; background-color:#AFF\"><b>$3</b></span>", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            sDetail = Regex.Replace(sDetail, @"((\r\n)*\[alert3\])(.*?)((\r\n)*\[\/alert3\])", "<span style=\"border:#090 2px solid;color:#090; font-size:16px; background-color:#BF9\"><b>$3</b></span>", RegexOptions.IgnoreCase | RegexOptions.Singleline);
            sDetail = ParseImg(sDetail,1);
            sDetail = parseFont(sDetail);
            sDetail = parseBold(sDetail);
            sDetail = ParseUrl(sDetail);
            sDetail = Regex.Replace(sDetail, @"((\r\n)*\[p\])(.*?)((\r\n)*\[\/p\])", "<p>$3</p>", RegexOptions.IgnoreCase | RegexOptions.Singleline);
        ...
        public static string HideDetail(string str, int hide, decimal usercredit)
        {
            if (hide == 0)
                return str;
        ...
        private static string ReplaceCustomTag(string sDetail, CustomEditorButtonInfo[] customeditorbuttoninfo)
        {
            ...
            //连接内部的AT不解析
            sDetail = Regex.Replace(sDetail, @"<a href=([^<>]+?)>([^<>]*?)\[atuser\](.+?)\[/atuser\]([^<>]*?)</a>", @"<a href=$1>$2@$3$4</a>", options);
            ...
            for (int i = 0; i < Editors.regexCustomTag.Length; i++)
            {
                ...
                for (int k = 0; k < customeditorbuttoninfo[i].Nest; k++)
                {
                    for (m = Editors.regexCustomTag[i].Match(sDetail); m.Success; m = m.NextMatch())
                    {
                        ...
                        //atuser 的urlencode的临时处理措施【BT修改】
                        if (customeditorbuttoninfo[i].Tag == "atuser")
                        {
                            tempReplacement = tempReplacement.Replace("=" + m.Groups[m.Groups.Count - 1].ToString(), "=" + Uri.EscapeUriString(m.Groups[m.Groups.Count - 1].ToString()));
                        }
