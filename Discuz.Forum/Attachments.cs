...
namespace Discuz.Forum
{
    ...
    public class Attachments
    {
        ...
        private static string ParseImageAttachContent(PostpramsInfo postPramsInfo, ShowtopicPageAttachmentInfo attachInfo, string fileSize)
        {
            ...
            //【BT修改】手机版 不直接加载图片postPramsInfo.Showimages != 1时为手机版
            if (postPramsInfo.Showimages != 1)
            {
                imageAttachTemplate = IMAGE_ATTACH_MTDOWNLOAD;

                if (postPramsInfo.Isforspace == 1)
                {
                    return string.Format("<a href=\"{0}attachment.aspx?attachmentid={1}\">[点击查看图片，{2}]</a>", forumPath, attachInfo.Aid, fileSize);
                }
                ...
                return string.Format("[收费图片附件，请登录电脑版查看，{0}]", fileSize);
            }
        ...
        private static string GetAttachReplacement(PostpramsInfo postPramsInfo, int allowGetAttach, ShowtopicPageAttachmentInfo attachInfo)
        {
            ...
            else if (attachInfo.Allowread == 1)
            {
                ...
                //【BT修改】，手机版 不显示收费附件 postPramsInfo.Showimages != 1为手机版，大小显示问题
                if(postPramsInfo.Showimages != 1)
                    attachInfoEm = string.Format("<em class=\"xg1\">({0}, 下载次数:{1})</em>", fileSize, attachInfo.Downloads);
                ...
                {
                    if (postPramsInfo.fgbtrss == 0)
                    {
                        replacement = Attachments.ParseImageAttachContent(postPramsInfo, attachInfo, fileSize);
                    }
                    else
                    {
                        replacement = string.Format("[img]upload/{0}[/img]", attachInfo.Filename);
                    } 
                }
                else
                {
                    ...
                    if (postPramsInfo.CurrentUserGroup.Radminid == 1 || postPramsInfo.CurrentUserid == attachInfo.Uid || attachInfo.Attachprice <= 0 || attachInfo.Isbought == 1)
                    {
                        ...
                        {
                            replacement = string.Format("[attachment]attachment.aspx?attachmentid={0}[/attachment]", attachInfo.Aid);
                        }
                    ...
                    {
                        //【BT修改】，手机版 不显示收费附件 postPramsInfo.Showimages != 1为手机版
                        if (postPramsInfo.Showimages != 1)
                        {
                            replacement = string.Format(ATTACH_INFO_DL, attachInfoDtimg + string.Format(ATTACH_INFO_DD,
                                            string.Format("{0}<br/><em class=\"xg1\">[收费附件，请登录电脑版查看]({1}, 下载次数{2})</em>", attachInfo.Attachment, fileSize, attachInfo.Downloads)));
                        }
                        else
                        {
                            if (postPramsInfo.fgbtrss == 0)
            ...
            else
            {
                if (postPramsInfo.CurrentUserid > 0)
                    ...
                    replacement = string.Format("<div class=\"hide\"><em><span class=\"attachnotdown\">你的下载权限 {0} 低于此附件所需权限 {1}, 你无权查看此附件</span></em></div>", postPramsInfo.Usergroupreadaccess, attachInfo.Readperm);
                else
        ...
        public static bool UpdateAttachment(AttachmentInfo[] attachmentArray, int topicId, int postId, PostInfo postInfo, ref StringBuilder returnMsg, int userId, GeneralConfigInfo config, UserGroupInfo userGroupInfo)
        {
            ...
            if (attachmentArray.Length > config.Maxattachments)
            {
                ...
                returnMsg.AppendFormat("您添加了{0}个图片/附件,多于系统设置的每个帖子{1}个.<br/>请重新编辑该帖并删除多余图片/附件.", attachmentArray.Length, config.Maxattachments);
                return false;
            }
