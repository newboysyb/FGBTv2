...
namespace Discuz.Forum
{
    ...
    public class TopicAdmins
    {
        ...
        public static int DeleteTopics(string topicList, int subTractCredits, bool reserveAttach)
        {
            ...
            {
                //【BT修改】只标记为删除，不从数据库中删除
                reval = SetTopicStatus(topicList, "displayorder", -1);
        ...
        public static int DeleteTopics(string topiclist, byte toDustbin, bool reserveAttach)
        {
            ...
            return SetTopicStatus(topiclist, "displayorder", -1);
        }
        ...
        public static int MoveTopics(string topiclist, int fid, int oldfid, int topicType)
        {
            ...
            foreach (string tid in topiclist.Split(','))
            {
                ...
                //////////////////////////////////////////////////////////////////////////
                //////////////////////////////////////////////////////////////////////////
                //【BT修改】移动种子，目的板块必须是种子类型所属板块

                TopicInfo topicinfo = Topics.GetTopicInfo(Utils.StrToInt(tid, 0));
                if (topicinfo != null && topicinfo.Tid == Utils.StrToInt(tid, 0) && topicinfo.SeedId > 0)
                {
                    PTSeedinfoShort seedinfo = PTSeeds.GetSeedInfoShort(topicinfo.SeedId);
                    if (PrivateBT.Type2Forum(seedinfo.Type) != fid)
                    {
                        return -1;
                        //PrivateBT.MoveSeedType(topicinfo.SeedId, PrivateBT.Forum2Type(fid));
                    }
                }
        ...
        public static int RatePosts(int tid, string postidlist, string score, string extcredits, int userid, string username, string reason)
        {
            ...
            for (int i = 0; i < tmpExtcreditslist.Length; i++)
            {
                ...
                if (tempExtc > 0 && tempExtc < extcreditslist.Length)
                {
                    //////////////////////////////////////////////////////////////////////////
                    ////////////////////////////////////////////////////////////////////////// 
                    //【BT修改】 评分是以MB为单位，加减时候要增加1024*1024倍
                    
                    if(tempExtc > 2) extcreditslist[tempExtc - 1] = TypeConverter.StrToFloat(tmpScorelist[i]) * 1048576f;
                    else extcreditslist[tempExtc - 1] = TypeConverter.StrToFloat(tmpScorelist[i]);

                    //【END BT修改】
                    //////////////////////////////////////////////////////////////////////////
                    ////////////////////////////////////////////////////////////////////////// 

                    //更新相应帖子的积分数、帖子中保存的仍为M数值
        ...
        public static void CancelRatePosts(string ratelogidlist, int tid, string pid, int userid, string username, int groupid, string grouptitle, int forumid, string forumname, string reason)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                ...
                //乘-1是要进行分值的反向操作
                int exttype = TypeConverter.ObjectToInt(dr["extcredits"]);
                float value = (-1) * TypeConverter.ObjectToFloat(dr["score"]);
                if(exttype > 2) value = value * 1048576f;
                Discuz.Data.Users.UpdateUserExtCredits(rateduserid, exttype, value);
            }
        ...
        public static void BumpTopics(string topiclist, int bumptype)
        {
            ...
            ////////////////////////////////////////////////////////////////////////// 
            //【BT修改】不能多个一起？
            //原程序：Discuz.Data.TopicAdmins.SetTopicsBump(topiclist, 0);
            {
                foreach (string tid in topiclist.Split(','))
                {
                    Discuz.Data.TopicAdmins.SetTopicsBump(tid, 0);
                }
            }
            //【END BT修改】
            //////////////////////////////////////////////////////////////////////////
