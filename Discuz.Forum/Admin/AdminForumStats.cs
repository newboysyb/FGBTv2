...
namespace Discuz.Forum
{
    ...
    public class AdminForumStats
    {
        ...
        {
            //////////////////////////////////////////////////////////////////////////
            //【BT修改】更新贴数和最后发帖时，同时更新所以父板块的贴数
            ForumInfo foruminfo = Forums.GetForumInfo(fid);
            foreach (string strfid in (foruminfo.Parentidlist + "," + fid.ToString()).Split(','))
            {
                int pfid = TypeConverter.StrToInt(strfid, -1);
                if (pfid > 0)
                {
                    if (pfid < 1)
                        return;
                    ...
                    string lastposter = "";
                    int todaypostcount = 0;
                    topiccount = Data.Topics.GetTopicCountOfForumWithSub(SubForumList(pfid));

                    postcount = GetPostsCountByFid(pfid, out todaypostcount);

                    IDataReader postreader = Data.Posts.GetLastPostByFid(pfid, Posts.GetPostTableName());
                    ...
                    Discuz.Data.Forums.UpdateForum(pfid, topiccount, postcount, lasttid, lasttitle, lastpost, lastposterid, lastposter, todaypostcount);
                }
            }
            //【END BT修改】
            //////////////////////////////////////////////////////////////////////////
