...
namespace Discuz.Forum
{
	...
	public class AdminVistLogs
	{
		...
		/// </summary>
		/// <returns></returns>
		public static bool DeleteLog(int userid , string ip)
		{
			try
			{
                Discuz.Data.AdminVisitLogs.DeleteLog();
                PTLog.InsertSystemLog(PTLog.LogType.AdminLog, PTLog.LogStatus.Critical, "DEL_ADMIN_LOG", "删除管理日志操作：--" + userid.ToString() + "--" + ip.ToString());
				return true;
			}
			catch
			{
		...
		/// <returns></returns>
        public static bool DeleteLog(string condition, int userid, string ip)
		{
			try
			{
                Discuz.Data.AdminVisitLogs.DeleteLog(condition);
                PTLog.InsertSystemLog(PTLog.LogType.AdminLog, PTLog.LogStatus.Critical, "DEL_ADMIN_LOG", "删除管理日志操作：--" + userid.ToString() + "--" + ip.ToString() + "--" + condition);
				return true;
			}
			catch
			{
