...
namespace Discuz.Forum
{
    public class Stats
    {
        ...
        /// <param name="sessionexists">是否在线？？？onlineuser是否存在</param>
        public static void UpdateStatCount(bool isguest, bool sessionexists)
        {
            ...
            //}
            ProcessStats ps = new ProcessStats(browser, os, visitorsadd, sessionexists, isguest);
            ps.Enqueue();
        }
        ...
        public static ShortUserInfo[][] GetExtsRankUserArray()
        {
            ...
            //for (i = 1; i < 9; i++)
            //{
            //    if (!(score[i] == string.Empty && i != 5 && i != 6)) arraysize++;
            //}
            ShortUserInfo[][] list = new ShortUserInfo[8][];
            arraysize = 0;
            for (i = 1; i < 9; i++, arraysize++)
            {
                if (!(score[i] == string.Empty && i != 5 && i != 6))
                {
                    list[arraysize] = GetUserArray("extcredits" + i.ToString());
                }
            }
            return list;
        ...
        /// <param name="type"></param>
        /// <returns></returns>
        public static string GetUserRankHtml(ShortUserInfo[] users, string type, int maxrows, int currentuid)
        {
            StringBuilder builder = new StringBuilder();
            string unit = "";
            ...
            foreach (ShortUserInfo u in users)
            {
                ...
                //【BT】排行，不显示0的数值
                if (type == "extcredits5" || type == "extcredits6")
                {
                    if ((type == "extcredits5" && u.Extcredits5 == 0M) || (type == "extcredits6" && u.Extcredits6 == 0M))
                    {
                        builder.AppendFormat("<li><a href=\"{1}\" target=\"_blank\"><em>{0}</em>{2}</a></li>", "&nbsp;", "&nbsp;", "&nbsp;");
                    }
                    else if (u.Uid == currentuid)
                    {
                        builder.AppendFormat("<li style=\"background-color:#FC6\"><a href=\"{1}\" target=\"_blank\"><em>{0}</em>{2}</a></li>", count + (unit == string.Empty ? string.Empty : " " + unit), Urls.UserInfoAspxRewrite(u.Uid), u.Username);
                    }
                    else
                    {
                        builder.AppendFormat("<li><a href=\"{1}\" target=\"_blank\"><em>{0}</em>{2}</a></li>", count + (unit == string.Empty ? string.Empty : " " + unit), Urls.UserInfoAspxRewrite(u.Uid), u.Username);
                    }
                }
                else
                {
        ...
        {
            public ProcessStats(string browser, string os, string visitorsadd, bool sessionexist, bool isguest)
            {
                _browser = browser;
                _os = os;
                _visitorsadd = visitorsadd;
                _sessionexist = sessionexist;
                _isguest = isguest;
            }
            protected string _browser;
            protected string _os;
            protected string _visitorsadd;
            protected bool _sessionexist;
            protected bool _isguest;
            ...
            {
                Discuz.Data.Stats.UpdateStatCount(this._browser, this._os, this._visitorsadd, this._sessionexist, this._isguest);
            }
        }
    }
}
