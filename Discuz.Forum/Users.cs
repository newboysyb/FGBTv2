...
namespace Discuz.Forum
{
    ...
    public class Users
    {
        ...
        public static void InsertUserFieldHistory(int uid, int moduid, string modip, string section, string originalvalue, string newvalue)
        {
            Discuz.Data.Users.InsertUserFieldHistory(uid, moduid, modip, section, originalvalue, newvalue);
        }
        ...
        public static float GetUserExtCredit(UserInfo userInfo, int extId)
        {
            ...
            switch (extId)
            {
                ...
                case 1: return (float)userInfo.Extcredits1;
                case 2: return (float)userInfo.Extcredits2;
                case 3: return (float)userInfo.Extcredits3;
                case 4: return (float)userInfo.Extcredits4;
                case 5: return (float)userInfo.Extcredits5;
                case 6: return (float)userInfo.Extcredits6;
                case 7: return (float)userInfo.Extcredits7;
                case 8: return (float)userInfo.Extcredits8;
        ...
        public static DataTable GetUserList(int pagesize, int pageindex, string column, string ordertype)
        {
            ...
            dt.Columns.Add("regmothed");
            dt.Columns.Add("reginfo");
            dt.Columns.Add("uploadstr");
            dt.Columns.Add("downloadstr");
            dt.Columns.Add("ratiostr");
            foreach (DataRow dataRow in dt.Rows)
            {
                ...
                decimal uploadvalue = TypeConverter.ObjectToDecimal(dataRow["extcredits3"]);
                decimal downloadvalue = TypeConverter.ObjectToDecimal(dataRow["extcredits4"]);

                dataRow["uploadstr"] = PTTools.Upload2Str(uploadvalue);
                dataRow["downloadstr"] = PTTools.Upload2Str(downloadvalue);
                dataRow["ratiostr"] = PTTools.Ratio2Str(uploadvalue, downloadvalue);

                if ((dataRow["ssouid"].ToString().Trim().ToLower() != "null" && dataRow["ssouid"].ToString().Trim() != "") ||
                    (dataRow["ssoname"].ToString().Trim().ToLower() != "null" && dataRow["ssoname"].ToString().Trim() != ""))
                {
                    dataRow["regmothed"] = "iBUAA";
                    dataRow["reginfo"] = dataRow["ssouid"].ToString().Trim() + "_" + dataRow["ssoname"].ToString().Trim();
                }
                else if ((dataRow["cngi_name"].ToString().Trim().ToLower() != "null" && dataRow["cngi_name"].ToString().Trim() != "") ||
                    (dataRow["cngi_school"].ToString().Trim().ToLower() != "null" && dataRow["cngi_school"].ToString().Trim() != ""))
                {
                    dataRow["regmothed"] = "CNGI";
                    dataRow["reginfo"] = dataRow["cngi_name"].ToString().Trim() + "_" + dataRow["cngi_school"].ToString().Trim();
                }
                else if (dataRow["inviteruid"].ToString().Trim().ToLower() != "null" && dataRow["inviteruid"].ToString().Trim() != "")
                {
                    dataRow["regmothed"] = "邀请";
                    int iuserid = TypeConverter.ObjectToInt(dataRow["inviteruid"], -1);
                    dataRow["reginfo"] = string.Format("<a href=\"userinfo-{0}.aspx\">{1}</a>", iuserid, Users.GetUserName(iuserid));
                }
                else
                {
                    dataRow["regmothed"] = "Free";
                    dataRow["reginfo"] = "开放注册";
                }
        ...
        //{
        //    int uid = Discuz.Data.Users.CheckPasswordAndSecques(username, password, originalpassword, ForumUtils.GetUserSecques(questionid, answer));
        ...
        //    if (uid > 0)
        //    {
        //        //新旧密码检测，必须为true原始密码模式
        //        PTUsers.CheckPasswordSHA512(username, password, true, !originalpassword, ForumUtils.GetUserSecques(questionid, answer), true);
        //    }
        ...
        //    {
        //        //新旧密码检测，必须为true原始密码模式
        //        PTUsers.CheckPasswordSHA512(username, password, true, !originalpassword, "", false);
        //    }
        ...
        //    {
        //        //新旧密码检测，必须为true原始密码模式
        //        PTUsers.CheckPasswordSHA512(uid, password, true, !originalpassword, "", false);
        //    }
        ...
        //    PTUsers.UpdatePasswordSHA512(uid, password, true);
        ...
        //        PTUsers.UpdatePasswordSHA512(userInfo.Uid, userInfo.Password, true);
        ...
        public static bool ValidateEmail(string email, int uid)
        {
            ...
            if (GeneralConfigs.GetConfig().Doublee == 0 && userid > 0 && uid != userid)
            {
                return false;
            }
            return true;
        ...
        public static void UpdateUserGroup(int uid, int groupId)
        {
            ...
            DatabaseProvider.GetInstance().UpdateUserOnlineInfo(groupId, uid);
        ...
        public static decimal GetUserExtCreditsByUserid(int uid, int extnumber)
        {
        ...
        public static int SendEmailByGroupidList(string groupidlist, int topnumber, ref int start_uid, string subject, string body)
        {
            ...
            foreach (DataRow dr in dt.Rows)
            {
                //【BT修改】原发送email逻辑错误，造成无法发送email
                if (dr["email"] != null && dr["email"].ToString().Trim() != "null" && dr["email"].ToString().Trim().IndexOf("ihome.buaa.edu.cn") < 0)
                {
