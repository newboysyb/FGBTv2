    ...
    public class UserAuthority
    {
        ...
        public static bool PostSpecialAuthority(ForumInfo forum, string type, ref string msg)
        {
            ...
            if (forum.Allowpostspecial > 0)
            {
                ...
                if (type == "seed" && (forum.Allowpostspecial & 64) != 64)
                {
                    msg = string.Format("当前版块 \"{0}\" 不允许发表文件交换种子", forum.Name);
                    return false;
                }
                if (type == "lottery" && (forum.Allowpostspecial & 128) != 128)
                {
                    msg = string.Format("当前版块 \"{0}\" 不允许发表博彩贴", forum.Name);
                    return false;
                }
            }
            return true;
        ...
        public static bool PostSpecialAuthority(UserGroupInfo usergroupinfo, string type, ref string msg)
        {
            ...
            // 验证用户是否有发起新文件交换种子的权限
            if (type == "seed" && !(usergroupinfo.Groupid < 4 || usergroupinfo.Groupid > 12))
            {
                msg = string.Format("您当前的身份 \"{0}\" 没有发文件交换种子的权限", usergroupinfo.Grouptitle);
                return false;
            }
            // 验证用户是否有发起博彩贴的权限
            if (type == "lottery" && !(usergroupinfo.Groupid < 4 || usergroupinfo.Groupid > 13))
            {
                msg = string.Format("您当前的身份 \"{0}\" 没有发博彩贴的权限", usergroupinfo.Grouptitle);
                return false;
            }
            return true;
        }
        ...
        /// <summary>
        /// 是否有编辑帖子的权限，此函数功能存在问题：并未审核版主所在板块2014.04.24
        /// </summary>
